...
"""Run-bound encrypted consumer for the fixed MM P03 operator-context test bundle.

Private source/test bytes are accepted only inside an X25519 + ChaCha20Poly1305 envelope
bound to one GitHub run. The public repository never stores plaintext private source.
The consumer validates an exact fixed payload shape, executes only the bundled unittest,
and emits a sanitized PASS/FAIL receipt with payload/output digests.
"""
...
SCHEMA = "p03-private-pytest-x25519-v1"
HARNESS = "mm_p03_operator_context_pytest_v1"
INFO = b"commandcenter-p03-private-pytest-v1"
ALLOWED_FILES = {
    "manifest.json",
...
def _validated_payload(plaintext: bytes) -> tuple[dict, bytes]:
    ...
    with tarfile.open(fileobj=io.BytesIO(plaintext), mode="r:gz") as tf:
        ...
        manifest_f = tf.extractfile(manifest_member)
    ...
    if manifest["schema"] != "p03-private-pytest-payload-v1":
    ...
    if int(manifest["mm_pr"]) != 222 or manifest["test_module"] != EXPECTED_TEST:
        raise RuntimeError("payload target mismatch")
    return manifest, payload_sha.encode("ascii")
...
def consume(envelope_path: Path, private_key_path: Path, expected_run_id: str) -> dict:
    ...
    manifest, _ = _validated_payload(plaintext)
    with tempfile.TemporaryDirectory(prefix="p03-private-") as tmp:
        root = Path(tmp)
        with tarfile.open(fileobj=io.BytesIO(plaintext), mode="r:gz") as tf:
            tf.extractall(root)
        source = (root / "strategy_health_operator_context.py").read_bytes()
        test = (root / "tests/test_strategy_health_operator_context.py").read_bytes()
        if hashlib.sha256(source).hexdigest() != manifest["source_sha256"]:
            raise RuntimeError("private source digest mismatch")
        if hashlib.sha256(test).hexdigest() != manifest["test_sha256"]:
            raise RuntimeError("private test digest mismatch")
        ...
        return {
            "schema": "p03-private-pytest-receipt-v1",
            "authority": "research_only",
            "harness": HARNESS,
            "mm_pr": 222,
...
def main() -> None:
    ...
    print("P03_PRIVATE_PYTEST_RECEIPT=" + json.dumps(receipt, sort_keys=True))
