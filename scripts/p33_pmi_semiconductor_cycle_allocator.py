...
def fold_stats(df, strat_col, control_col):
    ...
    for n,arr in enumerate(idx,1):
        x=df.iloc[arr]; a=cagr(x[strat_col]); b=cagr(x[control_col])
...
def main():
    prices={}; src={}
    for sym in ("SMH","QQQ","SPY"):
        prices[sym],src[sym]=yahoo(sym)
    pmi,src["NAPM"]=fred("NAPM")
    m=pd.concat({k:v.resample("ME").last() for k,v in prices.items()},axis=1).dropna()
    pmi_m=pmi.resample("ME").last().reindex(m.index,method="ffill")
    # Frozen causal macro state: prior month-end ISM manufacturing PMI > 50 selects SMH next month; otherwise QQQ.
    signal=(pmi_m>50).astype(int)
    rets=m.pct_change(); df=pd.DataFrame(index=m.index)
    df["smh_ret"]=rets["SMH"]; df["qqq_ret"]=rets["QQQ"]; df["spy_ret"]=rets["SPY"]
    df["signal_smh"]=signal.shift(1); df=df.dropna().copy()
    ...
    result={"schema":"research.p33_pmi_semiconductor_cycle_allocator.v1","hypothesis":"Prior month-end ISM manufacturing PMI above 50 selects SMH for next month; otherwise QQQ.","window":{"start":str(df.index.min().date()),"end":str(df.index.max().date()),"months":int(len(df))},"sources":src,"switches":int(switches.sum()),"controls":{},"cost_cases":{}}
    ...
    Path("artifacts").mkdir(exist_ok=True); Path("artifacts/p33_pmi_semiconductor_cycle_allocator.json").write_text(json.dumps(result,indent=2,sort_keys=True)+"\n")
    print("P33_RESULT="+json.dumps(result,sort_keys=True))
