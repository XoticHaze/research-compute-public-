...
def execute_options_snapshot(v:dict,root:Path,input_root:Path|None,artifact_root:Path|None,scanner_cls:Any)->dict:
    ...
    analyzer=getattr(scanner_cls,"analyze_snapshot_rows",None)
    if not callable(analyzer):
        raise CanonicalDispatchError("OPTIONS_SNAPSHOT_ANALYZE canonical owner analyze_snapshot_rows is unavailable")
    try:
        canonical=analyzer(
            rows,
        ...
        raise CanonicalDispatchError(f"OPTIONS_SNAPSHOT_ANALYZE canonical owner rejected request: {e}") from e
    if not isinstance(canonical,dict) or canonical.get("schema")!="mmibkr.options_snapshot_analysis.v1":
        raise CanonicalDispatchError("OPTIONS_SNAPSHOT_ANALYZE canonical owner returned invalid schema")
    policy=canonical.get("policy")
    safety=canonical.get("safety")
    analyzed=canonical.get("rows")
    summary=canonical.get("summary")
    if not isinstance(policy,dict) or not isinstance(safety,dict) or not isinstance(analyzed,list) or not isinstance(summary,dict):
        raise CanonicalDispatchError("OPTIONS_SNAPSHOT_ANALYZE canonical owner returned incomplete contract")
    for key in ("ibkr_acquisition","alpaca_acquisition","network_acquisition"):
        if policy.get(key) is not False:
            raise CanonicalDispatchError(f"OPTIONS_SNAPSHOT_ANALYZE canonical owner acquisition authority drift: {key}")
    if policy.get("snapshot_input_only") is not True or policy.get("deterministic_as_of") is not True:
        raise CanonicalDispatchError("OPTIONS_SNAPSHOT_ANALYZE canonical owner policy drift")
    if safety.get("research_only") is not True:
        raise CanonicalDispatchError("OPTIONS_SNAPSHOT_ANALYZE canonical owner research-only drift")
    for key in ("broker_submit","broker_cancel","broker_flatten","strategy_spec_write","runtime_activation","promotion_mutation","live_trading"):
        if safety.get(key) is not False:
            raise CanonicalDispatchError(f"OPTIONS_SNAPSHOT_ANALYZE canonical owner safety drift: {key}")
    ...
    public={k:deepcopy(value) for k,value in canonical.items() if k!="rows"}
    public["dataset"]={k:descriptor[k] for k in ("relative_path","sha256","bytes")}
    public["row_sample"]=deepcopy(analyzed[:50])
    public["analytics_sha256"]=sha(analyzed)
