...
SPY="SPY"; LOOK=252; SKIP=21; HOLD=21; TOP=3; COST_BPS=20.0; MIN_TRAIN=1008; FOLDS=6; PURGE=295
OUT=Path("results/sector_etf_momentum_rotation_r1.json")

def load(s):
 u=f"https://query1.finance.yahoo.com/v8/finance/chart/{s}?period1=915148800&period2=1798761600&interval=1d&events=history&includeAdjustedClose=true"
 r=Request(u,headers={"User-Agent":"Mozilla/5.0"})
 with urlopen(r,timeout=45) as z:p=json.loads(z.read().decode())
 rr=((p.get("chart") or {}).get("result") or [])
 if not rr: raise RuntimeError(f"{s}: yahoo no result")
 x=rr[0]; ts=x.get("timestamp") or []; ind=((x.get("indicators") or {}).get("adjclose") or [{}])[0].get("adjclose") or []
 if len(ts)!=len(ind): raise RuntimeError(f"{s}: timestamp/adjclose mismatch")
 vals=[(t,v) for t,v in zip(ts,ind) if v is not None and np.isfinite(float(v))]
 ser=pd.Series([float(v) for _,v in vals],index=pd.DatetimeIndex(pd.to_datetime([t for t,_ in vals],unit="s",utc=True)).normalize(),name=s,dtype=float).sort_index()
...
def main():
 ...
 for fi,(a,b) in enumerate(fs,1):
  ...
  while i<b-(HOLD+1):
   ...
   ew=float(np.mean([px[s].iloc[out]/px[s].iloc[ex]-1 for s in SECTORS])); spy=float(px[SPY].iloc[out]/px[SPY].iloc[ex]-1)
