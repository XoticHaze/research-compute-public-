...
hash-bound narrow source capsule to that recipient. This consumer decrypts only in
runner temp, executes the fixed research harness, then encrypts the private forecast/book
outputs to a separate return recipient. Public output is receipt-only.

Before each live generation the harness also replays the standalone observer as-of
2026-09-04. That private replay is returned encrypted so the authority can compare its
economic fields with the immutable original forward ledger before promotion/merge.
"""
...
def _observer(root: Path, output: Path, *, asof: str | None = None) -> None:
    ...
    cmd.extend(["--output", str(output.relative_to(root))])
    run = subprocess.run(
        cmd,
        cwd=root,
        env=env,
...
def _run_private_harness(root: Path) -> tuple[bytes, dict]:
    ...
    if str(parity.get("generation_bar_date")) != PARITY_ASOF:
        raise RuntimeError("parity replay generation date mismatch")
    ...
    if generation >= str(contract["prospective_start_signal_date"]):
        ...
        if book.returncode != 0:
            raise RuntimeError(f"three-book constructor failed rc={book.returncode}")
    ...
    receipt = {
        ...
        "parity_asof": PARITY_ASOF,
        "parity_prediction_sha256": hashlib.sha256(parity_path.read_bytes()).hexdigest(),
    ...
    with tarfile.open(fileobj=output_buf, mode="w:gz", format=tarfile.PAX_FORMAT) as tf:
        for arcname, path in [
            ("parity-2026-09-04.json", parity_path),
