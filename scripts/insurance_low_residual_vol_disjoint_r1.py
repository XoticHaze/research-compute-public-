...
def main():
 ...
 for fi,(a,b) in enumerate(folds(len(common)),1):
  ...
  while i<safe:
   ...
   k=max(1,int(np.ceil(len(scores)*.25)));chosen=sorted(scores,key=scores.get)[:k];ex=i+DELAY;out=ex+HOLD;sr=float(np.mean([px[s].iloc[out]/px[s].iloc[ex]-1 for s in chosen]));ir=float(px[IND].iloc[out]/px[IND].iloc[ex]-1);br=float(px[BROAD].iloc[out]/px[BROAD].iloc[ex]-1);netret=sr-COST/1e4
   rows.append({"fold":fi,"chosen":chosen,"net_bps":netret*1e4,"industry_excess_bps":(netret-ir)*1e4,"spy_excess_bps":(netret-br)*1e4});i+=HOLD
 fr=[]
 for fi in range(2,7):
