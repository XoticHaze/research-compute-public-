...
SEPARATOR_SOURCE_COMMIT = "current-main-20260911"
...
def derive(shared: bytes, data: bytes) -> bytes:
    return HKDF(algorithm=hashes.SHA256(), length=32,
                salt=hashlib.sha256(data).digest(), info=INFO).derive(shared)
...
def validate_payload(plaintext: bytes) -> dict:
    with tarfile.open(fileobj=io.BytesIO(plaintext), mode="r:gz") as tf:
        ...
        if names != ALLOWED_FILES:
            raise RuntimeError(f"payload file set mismatch: {sorted(names ^ ALLOWED_FILES)}")
    ...
    if manifest["schema"] != "mnq-w106-recovery-payload-v1" or manifest["harness"] != HARNESS or manifest["authority"] != "research_only":
        raise RuntimeError("manifest identity mismatch")
    if manifest["producer_source_commit"] != PRODUCER_SOURCE_COMMIT:
        raise RuntimeError("producer source commit mismatch")
    if not str(manifest["separator_source_commit"]):
        raise RuntimeError("separator source commit missing")
    if set(manifest["files_sha256"]) != (ALLOWED_FILES - {"manifest.json"}):
        raise RuntimeError("manifest file digest set mismatch")
    return manifest
...
def consume(envelope: Path, private_key_path: Path, run_id: str) -> dict:
    ...
    if set(env) != required or env["schema"] != SCHEMA or str(env["run_id"]) != str(run_id) or env["harness"] != HARNESS or env["authority"] != "research_only":
        raise RuntimeError("envelope identity mismatch")
    ...
    with tempfile.TemporaryDirectory(prefix="mnq-w106-recovery-") as td:
        ...
        for rel, expected in manifest["files_sha256"].items():
            ...
            if actual != expected:
                raise RuntimeError(f"private source digest mismatch: {rel}")
        ...
        bars = root / "mnq-12min.csv"
        out = root / "w106"
        separator = root / "separator.json"
        run_checked(["python", "scripts/research/mnq_crw_proven_bars_rebuild_20260901.py", "--source-root", str(data_root), "--output", str(bars)], root)
        ...
        if not replay.get("accepted_screen_parity_pass"):
            raise RuntimeError("accepted W106 screen parity failed")
        ...
        return {
            "schema": "mnq-w106-recovery-public-receipt-v1",
            "authority": "research_only",
            "harness": HARNESS,
            "run_id": str(run_id),
            "payload_sha256": hashlib.sha256(plaintext).hexdigest(),
            "producer_source_commit": manifest["producer_source_commit"],
            "separator_source_commit": manifest["separator_source_commit"],
            "mnq_data_commit": MNQ_DATA_COMMIT,
            "accepted_screen_parity_pass": True,
            "w106_metrics": replay.get("metrics"),
            "separator_events": sep.get("events"),
            "separator_aggregate": sep.get("aggregate"),
...
def main() -> None:
    ...
    a = p.parse_args()
    receipt = consume(Path(a.envelope), Path(a.private_key), a.run_id)
    print("MNQ_W106_RECOVERY_RECEIPT=" + json.dumps(receipt, sort_keys=True))
