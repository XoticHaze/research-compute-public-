...
import numpy as np
import pandas as pd
START="2000-01-01"; END="2026-09-08"; COSTS=(10,25,50)
...
def main():
    ...
    r=m.pct_change(); rel=(m["SMH"].pct_change(6)-m["QQQ"].pct_change(6)).shift(1)
    df=pd.DataFrame(index=m.index); df["smh"]=r["SMH"]; df["qqq"]=r["QQQ"]; df["spy"]=r["SPY"]; df["signal"]=(rel>0).astype(float); df=df.dropna()
    df["gross"]=np.where(df["signal"]>0,df["smh"],df["qqq"]); df["static"]=0.5*df["smh"]+0.5*df["qqq"]
    switches=df["signal"].diff().abs().fillna(1)
    out={"schema":"research.p36_semiconductor_relative_momentum_allocator.v1","hypothesis":"Prior 6-month SMH-minus-QQQ relative return above zero selects SMH next month; otherwise QQQ.","window":{"start":str(df.index.min().date()),"end":str(df.index.max().date()),"months":int(len(df))},"sources":src,"switches":int(switches.sum()),"controls":{},"cost_cases":{},"evaluation_integrity":{"common_last_daily_observation":str(common_last.date()),"incomplete_terminal_month_excluded":True}}
    ...
    Path("artifacts").mkdir(exist_ok=True); Path("artifacts/p36_semiconductor_relative_momentum_allocator.json").write_text(json.dumps(out,indent=2,sort_keys=True)+"\n"); print("P36_RESULT="+json.dumps(out,sort_keys=True))
