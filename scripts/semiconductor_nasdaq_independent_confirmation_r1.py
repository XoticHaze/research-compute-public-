...
OUT = Path("results/semiconductor_nasdaq_independent_confirmation_r1.json")
...
def _engineer(prices: dict[str, pd.Series], calendar: pd.DatetimeIndex):
    ...
    for symbol in (*TRAIN, *EXTERNAL):
        ...
        if symbol in TRAIN:
            df["survivor_cross_section_mom20_pct"] = surv.rank(axis=1, pct=True, method="average")[symbol]
        else:
...
def _frame(symbols, data, folds):
    ...
    for symbol in symbols:
        ...
        df["fold"] = fc
        use = df[(df.fold > 0) & df.guard_ok][
            ["symbol", "signal_i", "fold", *FEATURES, "enter20_after25_bps", "smh20_gross_bps", "mom20"]
        ]
        rows.append(use)
    return pd.concat(rows, ignore_index=True).replace([np.inf, -np.inf], np.nan).dropna(
        subset=FEATURES + ["enter20_after25_bps", "smh20_gross_bps", "mom20"]
    )
...
def main():
    ...
    for symbol in EXTERNAL:
        ...
        for fold in range(EVAL_FIRST_FOLD, FOLDS + 1):
            ...
            selected_all.extend(chosen.enter20_after25_bps.to_numpy(float).tolist())
