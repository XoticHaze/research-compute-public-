...
const MM_PRODUCT_HEAD='fdc9ba846d21bab365c74ad825902883810c3be4'
...
const handoff=Object.freeze({state:'HOLD_PRODUCT_EVIDENCE_OPEN',portfolio_authority_request_count:0,older_admission_may_release_handoff:false})
...
test('bind exact MM chronology-safe export head',()=>assert.equal(MM_PRODUCT_HEAD,'fdc9ba846d21bab365c74ad825902883810c3be4'))
...
test('portfolio handoff remains held while current P167 robustness is open',()=>{assert.equal(handoff.state,'HOLD_PRODUCT_EVIDENCE_OPEN');assert.equal(handoff.portfolio_authority_request_count,0);assert.equal(handoff.older_admission_may_release_handoff,false)})
