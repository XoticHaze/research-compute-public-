...
const MM_PRODUCT_HEAD='673a36a8b92539e7b0a348325c94bf63631e429e'
...
const handoff=Object.freeze({state:'HOLD_PRODUCT_EVIDENCE_OPEN',supported_representation:null,historical_supported_representation:'SPY_QQQ_TLT_GLD_DBC',portfolio_authority_request_count:0,older_admission_may_release_handoff:false})
...
test('bind exact MM chronology-safe export head',()=>assert.equal(MM_PRODUCT_HEAD,'673a36a8b92539e7b0a348325c94bf63631e429e'))
...
test('portfolio handoff remains held and current representation support is null while P167 is open',()=>{assert.equal(handoff.state,'HOLD_PRODUCT_EVIDENCE_OPEN');assert.equal(handoff.supported_representation,null);assert.equal(handoff.historical_supported_representation,'SPY_QQQ_TLT_GLD_DBC');assert.equal(handoff.portfolio_authority_request_count,0);assert.equal(handoff.older_admission_may_release_handoff,false)})
