...
function canonicalCandidate(raw) {
  ...
  for (const [key, value] of Object.entries(metrics)) {
    if (value === null || typeof value === 'string' || typeof value === 'boolean') {
      normalizedMetrics[String(key)] = value;
    } else {
      normalizedMetrics[String(key)] = finite(value);
    }
  }

  const reasons = Array.isArray(raw.reasons)
    ? raw.reasons.slice(0, 30).map((value) => text(value, 'reason', 1000))
    : [];
  const warnings = Array.isArray(raw.warnings)
    ? raw.warnings.slice(0, 30).map((value) => text(value, 'warning', 1000))
    : [];
  ...
  return {
    ...
    source: text(raw.source || 'UNKNOWN', 'source', 128).toUpperCase(),
    ...
    review_ready: raw.review_ready === true,
    sample: safeObject(raw.sample),
    metrics: normalizedMetrics,
    risk: safeObject(raw.risk),
    execution: safeObject(raw.execution),
    health: safeObject(raw.health),
...
export function validatePromotionCandidateSnapshot(node) {
  ...
  if (
    safety.broker_mutation_authority !== false
    || safety.live_execution_allowed !== false
    || safety.selected_runtime_write !== false
  ) {
  ...
  return {
    ...
    generated_at_utc: text(node.generated_at_utc, 'generated_at_utc', 128),
    candidates,
    policy: {
      research_to_arena_automatic_allowed: true,
      initial_calibration_broker_paper_review_required: true,
      live_enable_manual_required: true,
    },
    safety: {
...
function decisionCanonicalBody(body, candidate) {
  ...
  if ((action === 'DENY' || action === 'FEEDBACK') && !note) {
    throw new Error('operator_note_required');
  }
...
export async function storePromotionCandidates(storage, snapshot, publisher = {}) {
  ...
  const record = {
    schema: 'mmibkr.promotion_candidate_state.v1',
    stored_at_utc: new Date().toISOString(),
    snapshot: normalized,
    publisher: {
      repository: publisher.repository || null,
      workflow_ref: publisher.workflow_ref || null,
      run_id: publisher.run_id || null,
    },
  };
  ...
  if (
    ...
    || readback.snapshot?.schema !== CANDIDATE_SCHEMA
    || readback.snapshot?.candidates?.length !== normalized.candidates.length
  ) {
    throw new Error('promotion_candidate_readback_failed');
  }
  return {
    ok: true,
    schema: 'mmibkr.promotion_candidate_publish_receipt.v1',
...
export async function readPromotionState(storage) {
  ...
  const candidates = candidateRecord?.snapshot?.schema === CANDIDATE_SCHEMA
    ? candidateRecord.snapshot.candidates
    : [];
...
export async function recordPromotionDecision(storage, body, operator) {
  ...
  for (const row of state.decisions) {
    if (row.request_id === requestId && row.decision_id !== decisionId) {
      throw new Error('request_id_conflict');
    }
    if (
      canonical.action === 'APPROVE'
      && row.candidate_id === candidate.candidate_id
      && row.candidate_digest === candidate.candidate_digest
      && row.action === 'APPROVE'
      && row.current_stage === canonical.current_stage
      && row.target_stage === canonical.target_stage
    ) {
  ...
  const receipt = {
    ...
    ...canonical,
    disposition: canonical.action === 'APPROVE'
      ? 'APPROVED_FOR_' + canonical.target_stage
      : canonical.action === 'DENY'
        ? 'DENIED'
        : 'FEEDBACK_RECORDED',
    operator: {
      identity_sha256: operatorHash,
      authenticated: true,
    },
    authority: {
      decision_recorded: true,
      promotion_consumer_required: canonical.action === 'APPROVE',
      broker_mutation_authority: false,
      selected_runtime_write: false,
      live_execution_allowed: false,
    },
  ...
  const bounded = Array.from(new Set(index)).slice(-MAX_DECISIONS);
  await storage.put('promotion:decision-index', bounded);
  const readback = await storage.get('promotion:decision:' + decisionId);
  if (!readback || readback.decision_id !== decisionId) {
    throw new Error('promotion_decision_readback_failed');
  }
  return receipt;
}
...
export function machinePromotionProjection(state) {
  ...
  return {
    ok: true,
    schema: 'mmibkr.promotion_review_machine_read.v1',
    ...
    decisions: Array.isArray(state.decisions)
      ? state.decisions.map((row) => ({
          ...row,
          operator: row.operator?.identity_sha256
            ? { identity_sha256: row.operator.identity_sha256, authenticated: true }
            : { authenticated: false },
        }))
      : [],
    policy: state.policy || {},
...
export const promotionReviewContract = Object.freeze({
