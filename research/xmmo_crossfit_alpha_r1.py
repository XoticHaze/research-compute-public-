...
T=['XMMO','MDY','MTUM']; START=1356998400; END=1893456000; COST=0.001
...
def main():
 d={t:mr(fetch(t)) for t in T}; months=sorted(set(d['XMMO'])&set(d['MDY'])&set(d['MTUM'])); rows=[(m,d['XMMO'][m]-COST/12,d['MDY'][m],d['MTUM'][m]) for m in months]; years=sorted(set(m[0] for m in months)); held=[]; yr={}
 for y in years:
