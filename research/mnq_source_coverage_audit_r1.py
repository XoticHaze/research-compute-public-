...
def main():
  with tempfile.TemporaryDirectory(prefix='mnq-source-') as td:
    ...
    if not source_root.is_dir(): raise SystemExit('MNQ_SOURCE_SUBDIR_MISSING')
    rows=[]
    for d in sorted(source_root.glob('MNQ ??-??')):
