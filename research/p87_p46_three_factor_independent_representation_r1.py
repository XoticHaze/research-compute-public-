...
def main():
 ...
 for bps in (25,50):
  c3=f.g3-f.t3*bps/10000; c4=f.g4-f.t4*bps/10000; m3,m4,me=metrics(c3),metrics(c4),metrics(f.ew); pe,fe=folds(c3,f.ew); p4c,f4=folds(c3,c4); costs[str(bps)]={'three_factor':m3,'four_factor':m4,'matched_equal_weight':me,'spy':metrics(f.spy),'qqq':metrics(f.qqq),'excess_cagr_vs_equal_weight':m3['cagr']-me['cagr'],'excess_cagr_vs_four_factor':m3['cagr']-m4['cagr'],'positive_folds_vs_equal_weight':pe,'positive_folds_vs_four_factor':p4c,'folds_vs_equal_weight':fe,'folds_vs_four_factor':f4}
 a,b=costs['25'],costs['50']; ok=a['excess_cagr_vs_equal_weight']>0 and a['excess_cagr_vs_four_factor']>0 and a['positive_folds_vs_equal_weight']>=3 and b['excess_cagr_vs_equal_weight']>0 and b['excess_cagr_vs_four_factor']>0; out={'schema':'research.p87_p46_three_factor_independent_representation_r1','parent_ids':['P46','P87'],'scientific_contract':{'representation':list(SYMS),'three_factor':'6m momentum + SMA200 trend + 6m drawdown rank; low-vol removed because prior predeclared ablation identified it as dispensable','four_factor_comparator':'same representation with original P46 low-vol included','matched_control':'same-universe equal weight','costs_bps':[25,50],'no_new_factor_or_weight_tuning':True},'window':{'start':str(f.index.min().date()),'end':str(f.index.max().date()),'months':len(f)},'turnover':{'three_factor_annual':float(f.t3.mean()*12),'four_factor_annual':float(f.t4.mean()*12)},'costs':costs,'decision':'SUPPORTED_PARSIMONIOUS_THREE_FACTOR_REPLICATION' if ok else 'THREE_FACTOR_NOT_REPLICATED_KEEP_FOUR_FACTOR'}; Path('artifacts').mkdir(exist_ok=True); Path('artifacts/p87_p46_three_factor_independent_representation_r1.json').write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False)); print(json.dumps({'decision':out['decision'],'25':{'excess_vs_ew':a['excess_cagr_vs_equal_weight'],'excess_vs_four':a['excess_cagr_vs_four_factor'],'folds_vs_ew':a['positive_folds_vs_equal_weight'],'folds_vs_four':a['positive_folds_vs_four_factor']},'50':{'excess_vs_ew':b['excess_cagr_vs_equal_weight'],'excess_vs_four':b['excess_cagr_vs_four_factor']},'turnover':out['turnover'],'window':out['window']},sort_keys=True))
