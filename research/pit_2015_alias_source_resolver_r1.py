...
def norm_name(x):
    ...
    s=s.replace('&',' and '); s=re.sub(r'[^a-z0-9]+',' ',s)
    # deterministic representation-only suffix removal, never semantic token search
    toks=s.split()
    suffix={'inc','incorporated','corp','corporation','co','company','ltd','limited','plc','llc','group','holdings','holding'}
...
def wiki():
    ...
    for t in pd.read_html(StringIO(h.text)):
        ...
        sk=next((low[k] for k in low if k in ('symbol','ticker','tickersymbol')),None)
        ck=next((low[k] for k in low if 'cik' in k),None)
        nk=next((low[k] for k in low if k in ('security','company','companyname','name')),None)
        if sk and ck and nk:
...
namecol='name' if 'name' in pit.columns else None
if not namecol: raise SystemExit('PIT_NAME_COLUMN_MISSING')
pit['pit_name']=pit[namecol].astype(str); pit['name_key']=pit.pit_name.map(norm_name)
rows=[]
for ticker in FIXED:
    p=pit.loc[pit.ticker_norm==ticker]
    if len(p)!=1:
        rows.append({'pit_ticker':ticker,'status':'PIT_ROW_NOT_UNIQUE','pit_rows':int(len(p))}); continue
    pr=p.iloc[0]; cand=w.loc[w.name_key==pr.name_key]
    rec={'pit_ticker':ticker,'pit_name':pr.pit_name,'name_key':pr.name_key,'candidate_count':int(len(cand))}
    if len(cand)==1:
        wr=cand.iloc[0]; rec.update({'status':'EXACT_UNIQUE_CONTEMPORANEOUS_NAME_MATCH','wiki_2015_ticker':wr.wiki_ticker,'wiki_2015_name':str(wr.wiki_name),'wiki_2015_cik':str(wr.wiki_cik)})
    elif len(cand)==0: rec['status']='NO_EXACT_CONTEMPORANEOUS_NAME_MATCH'
    else: rec.update({'status':'AMBIGUOUS_CONTEMPORANEOUS_NAME_MATCH','candidates':cand[['wiki_ticker','wiki_name','wiki_cik']].astype(str).to_dict('records')})
    rows.append(rec)
resolved=sum(r['status']=='EXACT_UNIQUE_CONTEMPORANEOUS_NAME_MATCH' for r in rows)
all_pass=resolved==len(FIXED)
...
OUT.write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False)); print(json.dumps({'decision':out['decision'],'counts':out['counts'],'rows':rows},sort_keys=True))
