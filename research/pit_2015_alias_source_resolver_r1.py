...
def wiki():
    ...
    for t in pd.read_html(StringIO(h.text)):
        ...
        sk=next((low[k] for k in low if k in ('symbol','ticker','tickersymbol')),None); ck=next((low[k] for k in low if 'cik' in k),None); nk=next((low[k] for k in low if k in ('security','company','companyname','name')),None)
        if sk and ck and nk:
...
pit['pit_name']=pit['name'].astype(str); pit['name_key']=pit.pit_name.map(norm_name)
rows=[]
for ticker in FIXED:
    ...
    if len(cand)==1:
        wr=cand.iloc[0]; rec.update({'status':'EXACT_UNIQUE_CONTEMPORANEOUS_NAME_MATCH','wiki_2015_ticker':js(wr.wiki_ticker),'wiki_2015_name':js(wr.wiki_name),'wiki_2015_cik':js(wr.wiki_cik)})
    elif len(cand)==0: rec['status']='NO_EXACT_CONTEMPORANEOUS_NAME_MATCH'
    else:
        rec.update({'status':'AMBIGUOUS_CONTEMPORANEOUS_NAME_MATCH','candidates':[{'wiki_ticker':js(r.wiki_ticker),'wiki_name':js(r.wiki_name),'wiki_cik':js(r.wiki_cik)} for _,r in cand.iterrows()]})
    rows.append(rec)
...
OUT.write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False)); print(json.dumps({'decision':out['decision'],'counts':out['counts'],'rows':rows},sort_keys=True,allow_nan=False))
