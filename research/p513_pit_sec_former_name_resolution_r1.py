...
pit=pitindex.get_constituents(TARGET,index='sp500').copy();pit['ticker_norm']=pit.ticker.map(nt);pit['pit_name']=pit['name'].where(pit['name'].notna(),None)
ps=set(pit.ticker_norm.dropna());ws=set(w.ticker_norm.dropna());pit_only=sorted(ps-ws);wiki_only=sorted(ws-ps)
# Freeze exactly the two aliases accepted by P512's actual predeclared name rule.
exact={'CBRE':'CBG','WELL':'HCN'}
