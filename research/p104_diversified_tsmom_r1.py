...
def main():
 ...
 for i in range(LB+SKIP,len(m)-1):
  score=m.iloc[i-SKIP]/m.iloc[i-LB]-1; w=(score>0).astype(float).to_numpy()/len(U); r=(m.iloc[i+1]/m.iloc[i]-1).to_numpy(float); turn=float(.5*np.abs(w-prev).sum()); spy=float(m['SPY'].iloc[i+1]/m['SPY'].iloc[i]-1); rows.append({'date':m.index[i+1],'gross':float(w@r),'turn':turn,'matched':float(r.mean()),'spy':spy,'inv':float(w.sum())}); prev=w
