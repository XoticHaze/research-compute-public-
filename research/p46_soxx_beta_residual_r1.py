...
def main():
    ...
    f=pd.DataFrame(rec,columns=['date','candidate','matched','qqq','spy']).set_index('date'); active=f.candidate-f.matched; tests={}
    for label,g in [('full',f),('2022_forward',f[f.index>=pd.Timestamp('2022-01-01')])]:
      y=(g.candidate-g.matched); X=np.column_stack([g.qqq,g.spy]); r=beta.fit(y,X); r['bootstrap']=beta.boot(y,X,reps=4000,block=12,seed=20260909+len(g)); r['months']=len(g); tests[label]=r
