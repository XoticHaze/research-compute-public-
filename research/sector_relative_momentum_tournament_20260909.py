...
def evaluate(name, bench, stocks):
    ...
    for i in range(LOOKBACK+252,len(cal)-(H+DELAY),STEP):
        ...
        rec={'entry':cal[e],'exit':cal[x],'year':int(cal[e].year),'gross':gross,'equal':equal,'bench':float(px[bench].iloc[x]/px[bench].iloc[e]-1),'spy':float(px['SPY'].iloc[x]/px['SPY'].iloc[e]-1),'qqq':float(px['QQQ'].iloc[x]/px['QQQ'].iloc[e]-1)}; ds.append(rec)
    ...
    for cost in COSTS:
        ...
        for k,col in [('benchmark','bench'),('equal','equal'),('SPY','spy'),('QQQ','qqq')]:
            cr=cagr(df[col],days); ex=mm['strategy_cagr']-cr; yearly={str(y):float((net[df.year==y]-df.loc[df.year==y,col]).mean()*10000) for y in sorted(df.year.unique())}; mm[k]={'cagr':cr,'max_drawdown':maxdd(df[col]),'excess_cagr':ex,'mean_excess_bps_per_decision':float((net-df[col]).mean()*10000),'positive_years':sum(v>0 for v in yearly.values()),'year_count':len(yearly)}
...
out={'schema':'sector_relative_momentum_tournament.v1','contract':{'signal':'prior126d stock return minus sector ETF return','selection':'top3_of8','rebalance_step_days':STEP,'entry_delay_days':DELAY,'holding_days':H,'costs_bps':COSTS,'no_parameter_tuning':True},'results':results,'survivors':survivors,'decision':'HAS_SURVIVOR_REQUIRES_INDEPENDENT_HOLDOUT' if survivors else 'SECTOR_RELATIVE_MOMENTUM_NOT_SUPPORTED','research_only':True,'promotion_authority':False,'allocation_authority':False,'live_trading_change':False}
open('/tmp/result.json','w').write(json.dumps(out,sort_keys=True,indent=2)+'\n'); print('SECTOR_RELATIVE_MOMENTUM='+json.dumps(out,sort_keys=True))
