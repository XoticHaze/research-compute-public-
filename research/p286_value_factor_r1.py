...
PAIRS={'VLUE_vs_IWB':('VLUE','IWB'),'RPV_vs_SPY':('RPV','SPY')}; CONTROL='QQQ'; START='2013-01-01'; END='2026-09-10'; COST=10.0; WINDOWS={'2015':'2015-01-01','2020':'2020-01-01','2022':'2022-01-01'}
def metric(r):
...
res={'schema':'research.p286_value_factor_r1','parent':'P286','claim':'Two independent investable broad-US value-factor implementations earn persistent after-cost excess versus matched broad-universe controls without timing, tactical selection, or weight optimization.','parameters':{'pairs':PAIRS,'entry_exit_bps':COST,'windows':WINDOWS,'no_tuning':True},'results':results,'decision_rule':'SUPPORTED_CANDIDATE if both matched pairs have positive 2015+ excess with >=3/5 positive chronology folds and no >5pp drawdown penalty, both remain positive 2020+, and at least one remains positive 2022+. Otherwise classify mixed or not supported without product/window tuning. QQQ is opportunity context, not matched gate.','decision':'P286_VALUE_FACTOR_SUPPORTED_CANDIDATE' if ok else ('P286_VALUE_FACTOR_MIXED' if any(v['matched_excess_cagr']>0 for w in results.values() for v in w.values()) else 'P286_VALUE_FACTOR_NOT_SUPPORTED'),'limitations':['ETF methodologies and fees are embedded in adjusted returns','VLUE history begins 2013','scientific evidence only; no allocation/ranking/runtime/broker/live authority'],'boundaries':{'scientific_authority':True,'portfolio_ranking':False,'allocation_authority':False,'runtime':False,'broker':False,'live_trading':False}}
Path('research/artifacts').mkdir(parents=True,exist_ok=True); Path('research/artifacts/p286_value_factor_r1.json').write_text(json.dumps(res,indent=2,sort_keys=True,allow_nan=False)); print(json.dumps(res,sort_keys=True))
