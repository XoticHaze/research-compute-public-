...
def main():
    ...
    out={'schema':'research.p57_year_concentration_r1','parent':'P57','hypothesis':'The frozen parsimonious cross-asset momentum+trend top-2 sleeve has 25-bps matched-control excess distributed across calendar years rather than carried by a few exceptional years.','scientific_contract':{'economics':'unchanged P57 momentum+trend cross-asset top-2 monthly','cost_bps':BP,'matched_control':'same-universe equal weight over identical months','opportunity_controls':['SPY','QQQ'],'tests':['calendar-year persistence','top-positive-year share','remove top 1/2/3/5 contribution years'],'no_parameter_tuning':True},'source':{'normalized_price_panel_sha256':p57.base.source_hash(close)},'full':{'start':str(z.index.min().date()),'end':str(z.index.max().date()),'months':int(len(z)),'excess_cagr':cagr(z.candidate)-cagr(z.matched),'excess_cagr_vs_spy':cagr(z.candidate)-cagr(z.spy),'excess_cagr_vs_qqq':cagr(z.candidate)-cagr(z.qqq),'positive_calendar_year_fraction':positive_year_fraction,'top3_positive_excess_share':top3_share},'years':years,'remove_top_contribution_years':removals,'decision':'P57_EDGE_CALENDAR_CONCENTRATED' if concentrated else 'P57_EDGE_CALENDAR_BROAD'}
    Path('artifacts').mkdir(exist_ok=True); Path('artifacts/p57_year_concentration_r1.json').write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False)); print(json.dumps(out,sort_keys=True))
