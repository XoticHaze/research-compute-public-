...
def attention():
 ...
 x['yoy']=x['views'].pct_change(12); x['risk_off']=(x['yoy']>0).astype(float).shift(1); x.index=x.index.to_period('M').to_timestamp('M'); return x['risk_off'].dropna()

def prices():
 out={}
 for t in ['SPY','BIL']:
  h=yf.Ticker(t).history(start=START,auto_adjust=True,actions=False)
  if h.empty: raise RuntimeError(f'empty price history: {t}')
  s=h['Close'].copy(); s.index=pd.to_datetime(s.index).tz_localize(None); out[t]=s
...
def evaluate(df,a,b):
 z=df.loc[a:b].copy(); w=float(z['spy_w'].mean()); z['matched_control']=w*z['SPY_ret']+(1-w)*z['BIL_ret']; s=z['strategy']; c=z['matched_control']; se=cagr(s); ce=cagr(c)
 return {'strategy':stats(s),'matched_control':stats(c),'spy':stats(z['SPY_ret']),'bil':stats(z['BIL_ret']),'matched_excess_cagr':None if se is None or ce is None else se-ce,'switches':int(z['switch'].sum()),'spy_weight_mean':w}

def main():
 sig=attention(); px=prices(); m=px.resample('ME').last(); r=m.pct_change().dropna(); d=r.join(sig.rename('risk_off').reindex(r.index,method='ffill')).dropna(); d['spy_w']=1-d['risk_off']; d['switch']=d['spy_w'].diff().abs().fillna(0); d['SPY_ret']=d['SPY']; d['BIL_ret']=d['BIL']; d['strategy']=d['spy_w']*d['SPY_ret']+(1-d['spy_w'])*d['BIL_ret']-d['switch']*(COST_BPS/10000.0)
 overall=evaluate(d,START,'2026-12-31'); folds=[evaluate(d,a,b) for a,b in FOLDS]; pos=sum(1 for f in folds if f['matched_excess_cagr'] is not None and f['matched_excess_cagr']>0)
 decision='P544_SUPPORTED' if overall['matched_excess_cagr'] is not None and overall['matched_excess_cagr']>0 and pos>=2 else 'P544_NOT_SUPPORTED'
