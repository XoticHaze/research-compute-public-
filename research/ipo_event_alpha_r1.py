...
def fit(rows):
 X=[[1.0,r[2],r[3],r[4],r[5]] for r in rows]; y=[r[1] for r in rows]; p=5
 xtx=[[sum(x[i]*x[j] for x in X) for j in range(p)] for i in range(p)]; xty=[sum(x[i]*z for x,z in zip(X,y)) for i in range(p)]; return solve(xtx,xty)
def main():
 d={t:mr(fetch(t)) for t in T}; months=sorted(set(d['IPO'])&set(d['SPY'])&set(d['QQQ'])&set(d['IWM'])&set(d['MTUM'])); rows=[(m,d['IPO'][m]-COST/12,d['SPY'][m],d['QQQ'][m],d['IWM'][m],d['MTUM'][m]) for m in months]; years=sorted(set(m[0] for m in months)); held=[]; yr={}
 for y in years:
  train=[r for r in rows if r[0][0]!=y]; test=[r for r in rows if r[0][0]==y]
  if len(train)<72 or len(test)<6:continue
  b=fit(train); vals=[r[1]-(b[0]+b[1]*r[2]+b[2]*r[3]+b[3]*r[4]+b[4]*r[5]) for r in test]; yr[y]=100*12*statistics.mean(vals); held += [(r[0],v) for r,v in zip(test,vals)]
 if not held:out={'decision':'UNSCORED_INSUFFICIENT_CROSSFIT_HISTORY','months':len(months)}
 else:
  recent=[v for m,v in held if m>=(2022,1)]; metrics={'months':len(held),'years':len(yr),'full_residual_alpha_pct':100*12*statistics.mean(v for _,v in held),'recent_2022_residual_alpha_pct':100*12*statistics.mean(recent),'positive_year_fraction':sum(v>0 for v in yr.values())/len(yr),'positive_years':sum(v>0 for v in yr.values()),'worst_year_residual_pct':min(yr.values())}; passes={k:metrics[k]>=v for k,v in G.items()}; out={'schema':'cc.market_research.ipo_event_alpha.r1','experiment_id':'MR_IPO_EVENT_ALPHA_20260912_R1','claim':'IPO new-listing corporate-event selection delivers durable after-cost residual alpha beyond broad, growth, small-cap, and momentum exposures','frozen':{'target':'IPO','factors':['SPY','QQQ','IWM','MTUM'],'leave_one_calendar_year_out':True,'minimum_training_months':72,'cost_annual':COST,'gates':G,'forbidden_rescue':['factor substitution','date-window tuning','gate tuning','cost reduction','product substitution']},'metrics':metrics,'passes':passes,'decision':'PROMOTE_COMPONENT_EVIDENCE' if all(passes.values()) else 'REJECT_NO_RESCUE','year_residual_alpha_pct':yr}
 open('ipo-event-alpha-r1-result.json','w').write(json.dumps(out,indent=2)); print(json.dumps(out,indent=2))
