...
d=pd.DataFrame(z,columns=['date','pct','fwd']).drop_duplicates('date').set_index('date').sort_index(); sig=(d.pct<=.75).astype(int); turn=sig.ne(sig.shift()).astype(float); turn.iloc[0]=float(sig.iloc[0]); d['net']=sig*d.fwd-COST*turn
