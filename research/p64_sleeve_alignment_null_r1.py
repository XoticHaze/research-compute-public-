...
def last_complete_month_end(daily_index):
    last=pd.Timestamp(daily_index.max()).tz_localize(None) if pd.Timestamp(daily_index.max()).tzinfo else pd.Timestamp(daily_index.max())
    return last.to_period('M').start_time-pd.Timedelta(days=1)

def prepare():
    cross,cclose=p57.run(0); ind,iclose=p47.run(IND,FACTORS,0)
    cutoff=min(last_complete_month_end(cclose.index),last_complete_month_end(iclose.index))
    cross=cross.loc[cross.index<=cutoff]; ind=ind.loc[ind.index<=cutoff]
...
def main():
    f,cc,ic,cutoff=prepare(); out={'schema':'research.p64_sleeve_alignment_null_r1','parent':'P64','hypothesis':'P64 diversification benefit depends on genuine contemporaneous complementarity between its frozen cross-asset and independent-industry sleeves rather than arbitrary averaging of two return streams.','scientific_contract':{'component_cost_bps':BP,'sleeve_weights':[0.5,0.5],'null_replications':N,'matched_control':'50% crossasset EW + 50% independent-industry EW with the same circular shift applied to industry control','opportunity_control':'QQQ','windows':WINDOWS,'complete_months_only':True,'no_signal_factor_weight_or_parameter_tuning':True},'tests':{},'source':{'provider':'Yahoo Finance via yfinance; research-only','cross_panel_sha256':base.source_hash(cc),'industry_panel_sha256':base.source_hash(ic),'last_complete_month_end':str(cutoff.date())}}
