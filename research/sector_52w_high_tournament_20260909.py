...
LOOKBACK=252; H=20; DELAY=1; STEP=20; COSTS=(25.,50.,100.)
def maxdd(rs):
...
def evaluate(name, bench, stocks):
    ...
    for i in range(LOOKBACK,len(cal)-(H+DELAY),STEP):
        scores={s:float(px[s].iloc[i]/px[s].iloc[i-LOOKBACK:i+1].max()) for s in stocks}
...
out={'schema':'sector_52w_high_tournament.v1','contract':{'signal':'current adjusted price divided by trailing252d adjusted-price maximum','selection':'top3_of8','rebalance_step_days':STEP,'entry_delay_days':DELAY,'holding_days':H,'costs_bps':COSTS,'no_parameter_tuning':True},'results':results,'survivors':survivors,'decision':'HAS_SURVIVOR_REQUIRES_INDEPENDENT_HOLDOUT' if survivors else 'SECTOR_52W_HIGH_NOT_SUPPORTED','research_only':True,'promotion_authority':False,'allocation_authority':False,'live_trading_change':False}
open('/tmp/result.json','w').write(json.dumps(out,sort_keys=True,indent=2)+'\n'); print('SECTOR_52W_HIGH='+json.dumps(out,sort_keys=True))
