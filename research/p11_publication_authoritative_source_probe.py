...
SCHEMA = "p11.publication_authoritative_source_probe.v1"
...
USER_AGENT = "MM-IBKR-P11-publication-source-probe/1.0"
...
def _probe_symbol(symbol: str, start_date: str, max_pages: int = 3) -> dict[str, Any]:
    ...
    for page in range(1, max_pages + 1):
        query = urllib.parse.urlencode({"ticker": symbol, "startDate": start_date, "page": page, "limit": 100})
        url = f"{BASE_URL}?{query}"
        try:
            status, payload = _get_json(url)
        except urllib.error.HTTPError as exc:
            failure = {"kind": "HTTP_ERROR", "status": int(exc.code), "page": page, "url": url}
            break
        except Exception as exc:  # noqa: BLE001 - receipt needs exact provider failure
            failure = {"kind": "REQUEST_ERROR", "error": repr(exc), "page": page, "url": url}
            break

        data = payload.get("data")
        pagination = payload.get("pagination")
        if not isinstance(data, list) or not isinstance(pagination, dict):
            failure = {"kind": "SCHEMA_ERROR", "page": page, "url": url, "top_level_keys": sorted(payload)}
            break
    ...
    attribution_hits = sum(
        1
        for row in rows
        if symbol in {str(item or "").strip().upper() for item in (row.get("tickers") or [])}
    )
...
def build_receipt() -> dict[str, Any]:
    ...
    observed_history_days = (observed_at - oldest).total_seconds() / 86400.0 if oldest else 0.0
    ...
    schema_pass = bool(successful) and all(
        ...
        or (
            ...
            and all(value == 0 for value in (row.get("required_field_missing_counts") or {}).values())
        )
        for row in successful
    )
    both_sectors_present = any(row.get("symbol") in SEMICONDUCTORS and row.get("rows_observed", 0) > 0 for row in successful) and any(
        row.get("symbol") in HOMEBUILDERS and row.get("rows_observed", 0) > 0 for row in successful
    )
    history_pass = observed_history_days >= REQUIRED_HISTORY_DAYS - 2
    ...
    elif not both_sectors_present:
        decision = "REJECT_SECTOR_COVERAGE"
    elif not history_pass:
        decision = "REJECT_INSUFFICIENT_HISTORY"
    else:
    ...
    return {
        ...
        "frozen_gate": {
            ...
            "requires_both_sector_coverage": True,
        },
        "provider_access_pass": provider_access_pass,
        "schema_pass": schema_pass,
        "both_sectors_present": both_sectors_present,
        "observed_history_days": round(observed_history_days, 4),
...
def main() -> int:
    ...
    parser.add_argument("--output", default="research/results/p11_publication_authoritative_source_probe.json")
    ...
    print(json.dumps({
        ...
        "both_sectors_present": receipt["both_sectors_present"],
        "observed_history_days": receipt["observed_history_days"],
        "output": str(output),
    }, sort_keys=True))
    return 0
