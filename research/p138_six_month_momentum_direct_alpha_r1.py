...
def evaluate(f: pd.DataFrame, start: str, bps: int) -> dict:
    ...
    return {"window":{"start":str(g.index.min().date()),"end":str(g.index.max().date()),"months":len(g)},"six_month":mm,"equal_weight":em,"twelve_month":base.metrics(m12),"spy":base.metrics(g.spy),"qqq":base.metrics(g.qqq),"excess_cagr_vs_equal_weight":mm["cagr"]-em["cagr"],"excess_cagr_vs_twelve_month":mm["cagr"]-base.metrics(m12)["cagr"],"excess_cagr_vs_spy":mm["cagr"]-base.metrics(g.spy)["cagr"],"excess_cagr_vs_qqq":mm["cagr"]-base.metrics(g.qqq)["cagr"],"positive_folds_vs_equal_weight":folds(m6,ew),"five_strongest_m6_minus_ew_months_removed_cagr":base.metrics(m6.loc[keep])["cagr"]-base.metrics(ew.loc[keep])["cagr"],"annual_turnover":float(g.m6_turn.mean()*12)}
...
def main() -> None:
    ...
    if all(cells) and all(folds_ok) and all(trims): decision="SIX_MONTH_MOMENTUM_BROAD_DURABLE_ALPHA_SUPPORTED"
    elif sum(cells)>=8 and sum(folds_ok)>=4: decision="SIX_MONTH_MOMENTUM_ALPHA_MIXED_OR_CONCENTRATED"
    else: decision="SIX_MONTH_MOMENTUM_NOT_BROADLY_SUPPORTED"
