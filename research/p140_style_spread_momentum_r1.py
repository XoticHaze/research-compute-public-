...
def main():
    ...
    decision='P140_STYLE_SPREAD_MOMENTUM_SUPPORTED_FOR_INDEPENDENT_VALIDATION' if a['excess_matched']>0 and a['positive_matched_folds']>=3 and b['excess_matched']>0 and b['positive_matched_folds']>=3 and a['excess_spy']>0 else 'P140_STYLE_SPREAD_MOMENTUM_NOT_SUPPORTED'
    out={'schema':'research.p140_style_spread_momentum_r1','parent':'P140','hypothesis':'The prior completed three-month value-versus-growth return spread trends enough that selecting the leading style for the next month creates after-cost excess.','contract':{'signal':'At each completed month-end select the higher trailing-three-month return of VTV or VUG for the next month','assets':['VTV','VUG'],'lookback_months':3,'cost_bps':list(COSTS),'matched_control':'static 50/50 VTV/VUG','broad_control':'SPY','growth_opportunity_control':'QQQ','windows':list(windows),'folds':5,'competing_sign_adjudicator_for':'P139','no_lookback_threshold_weight_cost_or_asset_search':True},'source':{'provider':'Yahoo Finance via yfinance; research-only dynamic source, not promotion-grade','last_complete_month_end':str(cut.date()),'panel_sha256':hashlib.sha256(px.reset_index().to_csv(index=False,float_format='%.10g').encode()).hexdigest()},'tests':tests,'decision':decision}
    Path('artifacts').mkdir(exist_ok=True); Path('artifacts/p140_style_spread_momentum_r1.json').write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False)); print(json.dumps(out,sort_keys=True))
