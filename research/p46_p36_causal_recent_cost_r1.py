...
import p46_p36_causal_cost_breakeven_r1 as cost

def main():
    close=base.load(cost.ALL); ms=pd.Timestamp.now(tz='UTC').tz_localize(None).to_period('M').to_timestamp(); close=close.loc[close.index<ms]
    f=cost.gross(close).loc[pd.Timestamp('2022-01-01'):]; rows=[]
    for bp in range(0,301,5):
        cand=f.gross-f.turnover*bp/10000
        rows.append({'bps':bp,'excess_matched':cost.cagr(cand)-cost.cagr(f.matched),'excess_qqq':cost.cagr(cand)-cost.cagr(f.qqq)})
