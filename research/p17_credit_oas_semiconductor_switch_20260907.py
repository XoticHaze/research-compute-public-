...
def load_oas() -> pd.Series:
    ...
    values = pd.to_numeric(frame[OAS_SERIES], errors="coerce")
    series = pd.Series(values.to_numpy(), index=idx, name=OAS_SERIES).dropna().sort_index()
    series = series[~series.index.duplicated(keep="last")]
    if len(series) < 1000:
        raise RuntimeError(f"{OAS_SERIES}: insufficient observations {len(series)}")
    return series
...
def summary(rows: list[tuple[pd.Timestamp, float]]) -> dict[str, object]:
    ...
    for date, ret in rows:
        ...
        year = str(date.year)
        years[year] = years.get(year, 1.0) * (1.0 + ret)
    ...
    return {
        ...
        "return_over_volatility": None if ann_vol <= 0 else cagr / ann_vol,
        "year_returns": {year: value - 1.0 for year, value in sorted(years.items())},
    }
...
def main() -> None:
    ...
    for n, i in enumerate(month_ends[:-1]):
        ...
        credit_supportive = current_oas <= mean_oas and delta_oas <= 0.0
        selected = "SMH" if credit_supportive else "QQQ"
        decisions.append({
            "i": i,
            "j": j,
            "date": decision_date,
            "macro_cutoff": available.index[-1],
            "oas": current_oas,
            "oas_mean": mean_oas,
            "oas_delta": delta_oas,
            "selected": selected,
    ...
    for row in decisions:
        ...
        baselines["equal_SMH_QQQ"].append((
            frame.index[j],
            0.5 * float(frame["SMH"].iloc[j] / frame["SMH"].iloc[i] - 1.0)
            + 0.5 * float(frame["QQQ"].iloc[j] / frame["QQQ"].iloc[i] - 1.0),
        ))

    summaries = {
        "credit_switch": {str(int(cost)): summary(policy[cost]) for cost in COSTS_BPS},
        **{name: summary(rows) for name, rows in baselines.items()},
    }
    primary = summaries["credit_switch"][str(int(PRIMARY_COST_BPS))]
    smh = summaries["SMH"]
    qqq = summaries["QQQ"]
    spy = summaries["SPY"]
    equal = summaries["equal_SMH_QQQ"]

    common_years = sorted(
        set(primary["year_returns"])
        & set(smh["year_returns"])
        & set(qqq["year_returns"])
        & set(spy["year_returns"])
    )
    ...
    yearly = {
        year: {
            "vs_SMH": primary["year_returns"][year] - smh["year_returns"][year],
            "vs_QQQ": primary["year_returns"][year] - qqq["year_returns"][year],
            "vs_SPY": primary["year_returns"][year] - spy["year_returns"][year],
        }
    ...
    cost_robust = all(
        summaries["credit_switch"][str(int(cost))]["cagr"] > summaries["QQQ"]["cagr"]
        for cost in COSTS_BPS
    )
    supported = (
        primary["cagr"] > smh["cagr"]
        and primary["cagr"] > qqq["cagr"]
        and primary["cagr"] > equal["cagr"]
        and primary["max_drawdown"] >= smh["max_drawdown"]
        and len(full_years) >= 10
        and wins_smh / len(full_years) >= 0.50
        and wins_qqq / len(full_years) >= 0.60
        and cost_robust
    )
    ...
    output = {
        ...
        "research_only": True,
        "frozen_hypothesis": (
            "At each month-end, use only ICE BofA US High Yield OAS observations at least seven calendar days old. "
            "Hold SMH for the next month when OAS is at or below its trailing 252-observation mean and has not risen over "
            "63 observations; otherwise hold QQQ. Independent credit-state information should improve after-cost return "
            "versus SMH, QQQ, SPY and a static 50/50 SMH-QQQ opportunity set without using future macro observations."
        ),
        "sources": {
            "prices": {"provider": "Yahoo chart adjusted close", "query_host": "query1.finance.yahoo.com"},
            "macro": {"provider": "Federal Reserve Bank of St. Louis FRED", "series": OAS_SERIES, "meaning": "ICE BofA US High Yield Index Option-Adjusted Spread"},
        },
        "causality": {
            "publication_lag_days": PUBLICATION_LAG_DAYS,
            "macro_cutoff_rule": "latest FRED observation timestamp <= price decision timestamp minus seven calendar days",
            "trade_rule": "signal at month-end close, measure return to next month-end close",
        },
        "parameters": {
            "oas_mean_observations": OAS_MA_OBS,
            "oas_delta_observations": OAS_DELTA_OBS,
            "costs_bps": COSTS_BPS,
            "primary_cost_bps": PRIMARY_COST_BPS,
        },
        "matched_window": {
            "signal_start": frame.index[int(decisions[0]["i"])].isoformat(),
            "return_end": frame.index[int(decisions[-1]["j"])].isoformat(),
            "common_price_rows": len(frame),
            "monthly_decisions": len(decisions),
        },
        "deployment": {
            "SMH_months": smh_months,
            "QQQ_months": len(decisions) - smh_months,
            "SMH_fraction": smh_months / len(decisions),
            "mean_one_way_turnover": float(np.mean(turns)),
            "median_one_way_turnover": float(np.median(turns)),
        },
        "summaries": summaries,
        "primary_25bps_excess": {
            "cagr_vs_SMH": primary["cagr"] - smh["cagr"],
            "cagr_vs_QQQ": primary["cagr"] - qqq["cagr"],
            "cagr_vs_SPY": primary["cagr"] - spy["cagr"],
            "cagr_vs_equal_SMH_QQQ": primary["cagr"] - equal["cagr"],
            "max_drawdown_improvement_vs_SMH": primary["max_drawdown"] - smh["max_drawdown"],
            "full_years": len(full_years),
            "positive_years_vs_SMH": wins_smh,
            "positive_years_vs_QQQ": wins_qqq,
            "year_excess": yearly,
        },
