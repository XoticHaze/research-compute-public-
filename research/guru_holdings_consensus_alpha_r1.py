...
T=['GURU','SPY','VTV','MTUM']; START=1325376000; END=1893456000; COST=0.001
...
def main():
 d={t:mr(fetch(t)) for t in T}; months=sorted(set(d['GURU'])&set(d['SPY'])&set(d['VTV'])&set(d['MTUM'])); rows=[(m,d['GURU'][m]-COST/12,d['SPY'][m],d['VTV'][m],d['MTUM'][m]) for m in months]; years=sorted(set(m[0] for m in months)); held=[]; yr={}
 for y in years:
