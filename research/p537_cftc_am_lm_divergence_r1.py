...
BASE='https://publicreporting.cftc.gov/resource/gpe5-46if.json'; CODE='13874A'; START='2010-01-01'; COST=0.001; WINDOWS={'all':'2012-01-01','recent':'2020-01-01'}
def fetch():
 q=urlencode({'$where':f"cftc_contract_market_code='{CODE}' AND report_date_as_yyyy_mm_dd >= '{START}T00:00:00.000'",'$order':'report_date_as_yyyy_mm_dd ASC','$limit':'5000'}); req=Request(BASE+'?'+q,headers={'User-Agent':'XoticHaze-Research/1.0'}); rows=json.loads(urlopen(req,timeout=45).read().decode()); out=[]
 for r in rows:
  oi=float(r['open_interest_all']); d=pd.Timestamp(r['report_date_as_yyyy_mm_dd']).tz_localize(None); am=(float(r['asset_mgr_positions_long'])-float(r['asset_mgr_positions_short']))/oi; lm=(float(r['lev_money_positions_long'])-float(r['lev_money_positions_short']))/oi; out.append((d,am,lm))
...
def met(x):
 ...
 e=(1+x).cumprod(); c=float(e.iloc[-1]**(52/n)-1); dd=float((e/e.cummax()-1).min()); v=float(x.std(ddof=1)*math.sqrt(52)) if n>1 else 0; return {'weeks':n,'cagr':c,'maxdd':dd,'sharpe':float(x.mean()*52/v) if v else None}
cot=fetch(); cot['release']=cot.index+pd.Timedelta(days=3); cot['div']=cot.am-cot.lm; cot['pct']=np.nan
for i in range(104,len(cot)):
 hist=cot['div'].iloc[:i].dropna(); cot.iloc[i,cot.columns.get_loc('pct')]=float((hist<=cot['div'].iloc[i]).mean())
raw=yf.download('SPY',start=START,end=(pd.Timestamp.utcnow()+pd.Timedelta(days=1)).date().isoformat(),auto_adjust=True,progress=False,threads=False); close=(raw['Close']['SPY'] if isinstance(raw.columns,pd.MultiIndex) else raw['Close']).dropna(); rows=[]
...
d=pd.DataFrame(rows,columns=['date','pct','fwd']).drop_duplicates('date').set_index('date').sort_index(); signal=(d.pct>0.5).astype(int); gross=signal*d.fwd; turn=signal.ne(signal.shift()).astype(float); turn.iloc[0]=float(signal.iloc[0]); d['net']=gross-COST*turn
...
for name,start in WINDOWS.items():
 q=d.loc[d.index>=start].copy(); frac=float((q.pct>0.5).mean()); ctrl=q.fwd*frac; m=met(q.net); b=met(ctrl); m['exposure']=frac; m['matched_cagr']=b['cagr']; m['matched_excess_cagr']=m['cagr']-b['cagr']; folds=[]
