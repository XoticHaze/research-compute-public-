...
OUT=Path('semiconductor_hurdle_probability_ranker_20260905.json'); COST=200.0
def regressor(): return HistGradientBoostingRegressor(loss='squared_error',learning_rate=0.05,max_iter=120,max_leaf_nodes=8,max_depth=3,min_samples_leaf=80,l2_regularization=10.0,random_state=0)
def classifier(): return HistGradientBoostingClassifier(loss='log_loss',learning_rate=0.05,max_iter=120,max_leaf_nodes=8,max_depth=3,min_samples_leaf=80,l2_regularization=10.0,random_state=0)
...
def summarize(rows,key):
    ...
    top=np.asarray([r[f'{key}_top']-COST for r in rows],float); ew=np.asarray([r['ew']-COST for r in rows],float); smh=np.asarray([r['smh']-COST for r in rows],float); ic=[r[f'{key}_ic'] for r in rows if r[f'{key}_ic'] is not None]; clear=[r[f'{key}_clear_rate'] for r in rows]
...
def main():
    ...
    fs=base.folds(len(cal)); tr=base.frame(base.TRAIN,data,fs,base.FULL); decisions=[]; class_balance=[]
    for f in range(2,7):
        ...
        y=(train.target.to_numpy(float) > (COST-base.COST_TRAIN)).astype(int); positive=float(np.mean(y)); class_balance.append({'fold':f,'train_rows':len(train),'clear200_rate':positive})
        if positive<=0 or positive>=1: raise RuntimeError(f'fold {f}: degenerate hurdle target rate={positive}')
        admission=base.fit(train,base.FULL); direct=regressor(); direct.fit(train[base.FULL].to_numpy(float),train.target.to_numpy(float)); hurdle=classifier(); hurdle.fit(train[base.FULL].to_numpy(float),y); i=start; safe=stop-(base.DELAY+base.HOLD)
        while i<safe:
            ...
            for s in base.TRAIN:
                ...
                if admit<=0:continue
                ds=float(direct.predict(np.asarray([vals],float))[0]); hp=float(hurdle.predict_proba(np.asarray([vals],float))[0,1]); gross=float(prices[s].iloc[xi]/prices[s].iloc[ei]-1)*10000.0; c.append({'symbol':s,'direct':ds,'hurdle':hp,'gross':gross})
            if len(c)>=2:
                dc=sorted(c,key=lambda r:(-r['direct'],r['symbol']))[:2]; hc=sorted(c,key=lambda r:(-r['hurdle'],r['symbol']))[:2]; decisions.append({'fold':f,'direct_top':float(np.mean([r['gross'] for r in dc])),'hurdle_top':float(np.mean([r['gross'] for r in hc])),'ew':float(np.mean([r['gross'] for r in c])),'smh':float(prices['SMH'].iloc[xi]/prices['SMH'].iloc[ei]-1)*10000.0,'direct_ic':sp([r['direct'] for r in c],[r['gross'] for r in c]),'hurdle_ic':sp([r['hurdle'] for r in c],[r['gross'] for r in c]),'direct_clear_rate':float(np.mean([r['gross']>COST for r in dc])),'hurdle_clear_rate':float(np.mean([r['gross']>COST for r in hc]))})
            i+=base.HOLD
    folds=[]
    for f in range(2,7):
