...
T=['CALF','IWM','VBR','XSHQ']; START=1483228800; END=1893456000; COST=0.001
...
def main():
 d={t:mr(fetch(t)) for t in T}; months=sorted(set(d['CALF'])&set(d['IWM'])&set(d['VBR'])&set(d['XSHQ'])); rows=[(m,d['CALF'][m]-COST/12,d['IWM'][m],d['VBR'][m],d['XSHQ'][m]) for m in months]; years=sorted(set(m[0] for m in months)); held=[]; yr={}
 for y in years:
 ...
 if not held:out={'schema':'cc.market_research.calf_fcf_smallcap_alpha.r1','decision':'UNSCORED_INSUFFICIENT_CROSSFIT_HISTORY','overlap_months':len(months)}
 else:
  recent=[v for m,v in held if m>=(2022,1)]; metrics={'months':len(held),'years':len(yr),'full_residual_alpha_pct':100*12*statistics.mean(v for _,v in held),'recent_2022_residual_alpha_pct':100*12*statistics.mean(recent),'positive_year_fraction':sum(v>0 for v in yr.values())/len(yr),'positive_years':sum(v>0 for v in yr.values()),'worst_year_residual_pct':min(yr.values())}; passes={k:metrics[k]>=v for k,v in G.items()}; out={'schema':'cc.market_research.calf_fcf_smallcap_alpha.r1','claim':'CALF free-cash-flow small-cap selection delivers durable after-cost implementation alpha beyond small-cap, small-value, and small-quality exposures','frozen':{'target':'CALF','factors':['IWM','VBR','XSHQ'],'leave_one_calendar_year_out':True,'cost_annual':COST,'gates':G},'metrics':metrics,'passes':passes,'decision':'PROMOTE' if all(passes.values()) else 'REJECT_NO_RESCUE','year_residual_alpha_pct':yr}
 open('calf-fcf-smallcap-alpha-r1-result.json','w').write(json.dumps(out,indent=2)); print(json.dumps(out,indent=2))
