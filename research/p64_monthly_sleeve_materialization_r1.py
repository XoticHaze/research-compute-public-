...
def load(syms):
 d=yf.download(list(dict.fromkeys((*syms,'SPY','QQQ'))),start='2005-01-01',auto_adjust=True,progress=False,threads=False)['Close'].dropna(how='all').astype(float); return d
...
def sleeve(close,syms,topk):
 ...
 for i,dt in enumerate(m.index[:-1]):
  ...
  chosen=list(score.sort_values(ascending=False,kind='mergesort').head(topk).index); w={s:(1/topk if s in chosen else 0.) for s in syms}; turn=.5*sum(abs(w[s]-prev[s]) for s in syms); gross=sum(w[s]*float(r[s]) for s in syms); rows.append({'date':pd.Timestamp(idx[z]),'gross':gross,'net':gross-turn*BP/10000,'matched':float(r.mean()),'turnover':turn,'chosen':'|'.join(chosen)}); prev=w
 return pd.DataFrame(rows).set_index('date'),cutoff

def main():
 cc=load(CROSS); ic=load(IND); cf,ccut=sleeve(cc,CROSS,2); inf,icut=sleeve(ic,IND,3); ix=cf.index.intersection(inf.index); q=pd.DataFrame(index=ix); q['p57_cross_net_50bps']=cf.loc[ix,'net']; q['p52_industry_net_50bps']=inf.loc[ix,'net']; q['p64_net_50bps']=.5*q.p57_cross_net_50bps+.5*q.p52_industry_net_50bps; q['matched_blend']=.5*cf.loc[ix,'matched']+.5*inf.loc[ix,'matched']; q['qqq']=cc['QQQ'].pct_change(fill_method=None).reindex(ix); q=q.dropna(); q.index.name='return_date'; Path('artifacts').mkdir(exist_ok=True); q.to_csv('artifacts/p64_monthly_sleeve_returns_5day_50bps.csv')
