...
def build():
    ...
    m=d.resample('ME').last(); vol=(d.pct_change(fill_method=None).rolling(126,min_periods=100).std(ddof=0)*math.sqrt(252)).resample('ME').last(); trend=(d/d.rolling(200,min_periods=160).mean()-1).resample('ME').last(); dd=(d/d.rolling(126,min_periods=100).max()-1).resample('ME').last(); mom=m.pct_change(6)
    ...
    for i,dt in enumerate(m.index[:-1]):
        ...
        block=pd.DataFrame({'symbol':list(U),'score':[float(score[s]) for s in U]}).sort_values(['score','symbol'],ascending=[False,True])
        chosen=block.head(2).symbol.tolist(); nxt=m.index[i+1]; rr=m.loc[nxt,list(U)]/m.loc[dt,list(U)]-1
...
def main():
    q,d,cut=build(); windows={'2015':'2015-01-01','2020':'2020-01-01','2022':'2022-01-01'}; tests={w:{str(c):evaluate(q.loc[pd.Timestamp(s):],c) for c in COSTS} for w,s in windows.items()}; thresholds={}
    for w in windows: thresholds[w]={k:max([c for c in COSTS if tests[w][str(c)][k]>0],default=None) for k in ('excess_equal_weight','excess_qqq','excess_spy')}
