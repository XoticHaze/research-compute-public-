...
BENCH='IGV'; STOCKS=('SNPS','PANW','FTNT','VRSN','AKAM','CTSH','TTWO','CHKP'); LOOKBACK=20; H=20; DELAY=1; STEP=20; COSTS=(25.,50.,100.)
def maxdd(rs):
...
coverage={s:{'rows':int(len(raw[s])),'first':raw[s].timestamp.iloc[0].isoformat() if len(raw[s]) else None,'last':raw[s].timestamp.iloc[-1].isoformat() if len(raw[s]) else None} for s in syms}
...
if len(cal)<1800 or any(v.isna().any() for v in px.values()):
    out={'schema':'software_reversal_holdout.v1','decision':'CALENDAR_INSUFFICIENT','calendar_rows':len(cal),'source_coverage':coverage,'research_only':True,'live_trading_change':False}; open('/tmp/result.json','w').write(json.dumps(out,sort_keys=True,indent=2)+'\n'); print('SOFTWARE_REVERSAL_HOLDOUT='+json.dumps(out,sort_keys=True)); raise SystemExit
ds=[]
...
out={'schema':'software_reversal_holdout.v1','contract':{'parent_signal':'prior20d stock-minus-IGV relative return; select bottom3','holdout_names':STOCKS,'selection':'bottom3_of8','rebalance_step_days':STEP,'entry_delay_days':DELAY,'holding_days':H,'costs_bps':COSTS,'no_parameter_tuning':True,'source_feasibility_replacement':'EA->CTSH after EA source exposed only six rows; no economic result observed before replacement'},'calendar_first':cal[0].isoformat(),'calendar_last':cal[-1].isoformat(),'first_entry':df.entry.iloc[0].isoformat(),'last_exit':df.exit.iloc[-1].isoformat(),'decisions':len(df),'metrics':metrics,'checks':checks,'source_coverage':coverage,'decision':'INDEPENDENT_HOLDOUT_SUPPORTED' if all(checks.values()) else 'INDEPENDENT_HOLDOUT_REJECTS_PARENT','research_only':True,'promotion_authority':False,'allocation_authority':False,'live_trading_change':False}
