...
def main():
 q=yf.download(['QQQ'],start='2000-01-01',auto_adjust=True,progress=False,threads=False); o=q['Open']['QQQ'].astype(float); c=q['Close']['QQQ'].astype(float); f=pd.DataFrame({'open':o,'close':c}).dropna(); f['cc']=f.close/f.close.shift(1)-1; f['prior_cc']=f.cc.shift(1); f['intraday']=f.close/f.open-1; f=f.dropna(); f['active']=f.prior_cc<0; f['gross']=np.where(f.active,f.intraday,0.0); f['always_intraday']=f.intraday
 out={'schema':'research.p97_prior_day_reversal_r1','parent':'P97','hypothesis':'A negative prior QQQ close-to-close return predicts next-day intraday mean reversion strongly enough to improve on always holding QQQ open-to-close after realistic round-trip costs.','contract':{'signal':'known before open: prior QQQ close-to-close return < 0','return':'next-day open-to-close when active, cash otherwise','costs_bps_roundtrip':list(C),'matched_control':'always hold every QQQ open-to-close','opportunity_control':'QQQ close-to-close','windows':['full','2015','2020'],'folds':5,'no_parameter_search':True},'tests':{}}
 for name,start in [('full',None),('2015','2015-01-01'),('2020','2020-01-01')]:
  ...
  out['tests'][name]=d
 z=out['tests']['2020']['costs']['5']; out['decision']='P97_PRIOR_DAY_REVERSAL_SUPPORTED' if z['excess_cagr_vs_always_intraday']>0 and z['positive_matched_folds']>=3 and z['sharpe_delta_vs_always_intraday']>0 else 'P97_PRIOR_DAY_REVERSAL_NOT_SUPPORTED'; Path('artifacts').mkdir(exist_ok=True); Path('artifacts/p97_prior_day_reversal_r1.json').write_text(json.dumps(out,indent=2)); print(json.dumps(out))
