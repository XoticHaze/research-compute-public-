...
LEARNING_PARENT = "F1B-NQ-DATED-CORPUS-RELEASE"
EXPERIMENT_ID = "NQ-DATED-TREND-TRANSPORT-R1"
...
def build_events(path: Path):
    ...
    out = []
    # Non-overlapping 5-active-session decisions after a fixed 20-active-session lookback.
    i = LOOKBACK
...
def main():
    files = sorted(Path("input/staging").glob("**/NQ-*/1Day.csv"))
    if not files:
        raise RuntimeError("no released NQ dated-contract files found")
    ...
    positive_contract_fraction = (
        sum(v > 0 for v in contract_excess.values()) / len(contract_excess)
        if contract_excess else 0.0
    )
    ...
    result = {
        "schema": "public.nq_dated_trend_transport_r1.v1",
        "experiment_id": EXPERIMENT_ID,
        "inherited_learning_ids": [LEARNING_PARENT],
        "claim_tested": "A fixed 20-active-session NQ trend sign predicts the next 5 active sessions across independently released dated contracts and beats an always-long matched opportunity after a 10 bp normalized round-trip cost without constructing a continuous series.",
        ...
        "forbidden_parameter_rescue": [
            "lookback change", "holding-period change", "cost reduction", "exclude bad contract years",
            "volume/open-interest threshold tuning", "select only favorable contract months"
        ],
    ...
    Path("nq-dated-trend-transport-r1-result.json").write_text(
