...
T=['SPMO','SPY','MTUM']; START=1420070400; END=1893456000; COST=0.001
...
def fit(rows):
 ...
 b1=(cy1*c22-cy2*c12)/det; b2=(cy2*c11-cy1*c12)/det; a=my-b1*m1-b2*m2; return a,b1,b2
def main():
 d={t:mr(fetch(t)) for t in T}; months=sorted(set(d['SPMO'])&set(d['SPY'])&set(d['MTUM'])); rows=[(m,d['SPMO'][m]-COST/12,d['SPY'][m],d['MTUM'][m]) for m in months]; years=sorted(set(m[0] for m in months)); held=[]; yr={}
 for y in years:
 ...
 if not held:
  ...
  full=100*12*statistics.mean(v for _,v in held); recent=[v for m,v in held if m>=(2022,1)]; recent_a=100*12*statistics.mean(recent) if recent else None; metrics={'months':len(held),'years':len(yr),'full_residual_alpha_pct':full,'recent_2022_residual_alpha_pct':recent_a,'positive_year_fraction':sum(v>0 for v in yr.values())/len(yr),'positive_years':sum(v>0 for v in yr.values()),'worst_year_residual_pct':min(yr.values())}; passes={k:metrics[k]>=v for k,v in G.items()}; out={'schema':'cc.market_research.spmo_crossfit_alpha.r1','claim':'SPMO retains durable implementation alpha beyond S&P 500 beta and generic momentum exposure','inherited_learning':['P300 classified SPMO/XMMO matched-excess relationship as high redundancy','XMMO and IDMO crossfit independent-alpha claims rejected under prospectively frozen beta plus generic-momentum factors'],'frozen':{'target':'SPMO','factors':['SPY','MTUM'],'leave_one_calendar_year_out':True,'cost_annual':COST,'gates':G},'metrics':metrics,'passes':passes,'decision':'PROMOTE' if all(passes.values()) else 'REJECT_NO_RESCUE','year_residual_alpha_pct':yr}
 open('spmo-crossfit-alpha-r1-result.json','w').write(json.dumps(out,indent=2)); print(json.dumps(out,indent=2))
