...
for s in SYMS:
    try:
        d=yf.download(s,start=START,end=END,auto_adjust=True,progress=False,threads=False); c=d['Close']; c=c.iloc[:,0] if isinstance(c,pd.DataFrame) else c; c=c.dropna(); series[s]=c
        ...
        series[s]=pd.Series(dtype=float); out['symbols'][s]={'rows':0,'first':None,'last':None,'error':repr(e)}
for pair in [('PUTW','SPY'),('PUTW','BIL'),('SPY','BIL')]:
    z=pd.concat([series[pair[0]],series[pair[1]]],axis=1).dropna(); out['intersections']['_'.join(pair)]={'daily_rows':int(len(z)),'first':str(z.index.min().date()) if len(z) else None,'last':str(z.index.max().date()) if len(z) else None}
...
if len(monthly):
    ex_put=monthly.PUTW-monthly.BIL; ex_spy=monthly.SPY-monthly.BIL; beta=(ex_put.rolling(36,min_periods=24).cov(ex_spy)/ex_spy.rolling(36,min_periods=24).var()).shift(1); matched=beta*monthly.SPY+(1-beta)*monthly.BIL; z=pd.concat([monthly.PUTW,matched,beta],axis=1).dropna(); out['monthly_joint_rows']=int(len(monthly)); out['causal_matched_rows']=int(len(z)); out['causal_first']=str(z.index.min().date()) if len(z) else None
else: out['monthly_joint_rows']=0; out['causal_matched_rows']=0; out['causal_first']=None
...
out['decision']=decision; out['boundaries']={'scientific_data_authority':True,'model_inference':False,'portfolio_ranking':False,'allocation_authority':False,'runtime':False,'broker':False,'live_trading':False}
