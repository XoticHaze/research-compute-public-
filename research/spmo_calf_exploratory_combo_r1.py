...
def equal_rebalanced(r,cols):
 w=pd.DataFrame(0.5,index=r.index,columns=cols); turn=w.diff().abs().sum(axis=1)/2; turn.iloc[0]=0.5; return (w*r[cols]).sum(axis=1)-turn*COST,turn
def folds(a,b):
