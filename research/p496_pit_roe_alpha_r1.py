...
OUT=Path('research/artifacts/p496_pit_roe_alpha_r1.json'); OUT.parent.mkdir(parents=True,exist_ok=True)
def hist(target):
 ...
 for concept in ['NetIncomeLoss','ProfitLoss']:
  for v in g.get(concept,{}).get('units',{}).get('USD',[]):
   if v.get('form')=='10-K' and v.get('start') and v.get('end') and v.get('filed') and v['filed']<=asof and v['end']<=asof:
    ...
    if 300<=days<=430 and v.get('val') is not None: ni.append((v['filed'],v['end'],float(v['val']),concept,v.get('accn')))
 if not ni:return None
 n=max(ni,key=lambda x:(x[0],x[1])); end=n[1]; eq=[]
 for concept in ['StockholdersEquity','StockholdersEquityIncludingPortionAttributableToNoncontrollingInterest']:
  for v in g.get(concept,{}).get('units',{}).get('USD',[]):
   if v.get('form')=='10-K' and not v.get('start') and v.get('end')==end and v.get('filed') and v['filed']<=asof and v.get('val') is not None: eq.append((v['filed'],float(v['val']),concept,v.get('accn')))
 if not eq:return None
 e=max(eq,key=lambda x:x[0])
 if e[1] <= 0:return None
 return {'roe':n[2]/e[1],'ni':n[2],'equity':e[1],'period_end':end,'ni_filed':n[0],'equity_filed':e[0],'equity_concept':e[2]}
...
for y in YEARS:
 ...
 usable=sorted(usable,key=lambda a:a['roe']); k=max(1,len(usable)//4); top=usable[-k:] if usable else []
...
decision='PIT_ROE_ALPHA_SUPPORTED' if supported else ('PIT_ROE_ALPHA_NOT_SUPPORTED' if all_ready else 'PIT_ROE_ALPHA_DATA_NOT_READY')
out={'schema':'research.p496_pit_roe_alpha_r1.v1','workload_id':'P496_PIT_ROE_ALPHA_R1','parent':'PIT_FUNDAMENTAL_CROSS_SECTIONAL_SELECTION','claim':'Materially different causal capital-efficiency discriminator after ROA rejection and sparse leverage/margin coverage: on fixed June-30 historical S&P universes and deterministic 20-name samples, rank by latest annual 10-K net-income/positive-stockholders-equity filed by rebalance date; compare top quartile forward 12-month adjusted return after fixed costs against equal-weight same-universe sample.','contract':{'years':YEARS,'sample_n':N,'sample_rule':'20 evenly spaced after ticker sort','feature':'latest causal annual 10-K NetIncomeLoss-or-ProfitLoss divided by positive StockholdersEquity at same fiscal period end','portfolio':'top quartile ROE','matched_control':'equal-weight same deterministic historical sample','endpoint_cost_bps':25,'annual_feature_and_price_coverage_min':.80,'all_years_required':True},'cohorts':cohorts,'summary':{'ready_years':len(valid),'positive_excess_years':pos,'candidate_compound_cagr':cg,'control_compound_cagr':bg,'after_cost_excess_cagr':ex},'decision_rule':'Support only if every fixed year clears >=80% causal feature and price coverage, >=5/7 annual cohorts have positive after-cost same-universe excess, and compounded candidate CAGR exceeds control. Otherwise reject or classify data-not-ready; no rescue.','decision':decision,'boundaries':{'scientific_authority':True,'portfolio_ranking':False,'allocation_authority':False,'runtime':False,'broker':False,'live_trading':False}}
