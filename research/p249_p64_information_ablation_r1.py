...
def rank_frame(symbols,topk,features):
    mm=c[list(symbols)].resample('ME').last()
    mom=mm.pct_change(6)
    ...
    for dt in mm.index:
        ...
        rows.append((nxt,sum(w[s]*float(rr[s]) for s in symbols)-to*COST_BP/10000,float(rr.mean())))
        prev=w
    q=pd.DataFrame(rows,columns=['date','ret','ctl']).set_index('date'); q.index=q.index.to_period('M').to_timestamp('M'); return q

def p64(features):
    a=rank_frame(CROSS,2,features); b=rank_frame(IND,3,features); ix=a.index.intersection(b.index)
    return pd.DataFrame({'ret':.5*a.loc[ix].ret+.5*b.loc[ix].ret,'ctl':.5*a.loc[ix].ctl+.5*b.loc[ix].ctl},index=ix)
...
def p36():
    ...
    for i in range(len(labs)-1):
        ...
        rows.append((idx[i1],w*ar+(1-w)*qr-to*COST_BP/10000,.5*(ar+qr))); prev=w
    q=pd.DataFrame(rows,columns=['date','ret','ctl']).set_index('date'); q.index=q.index.to_period('M').to_timestamp('M'); return q

svr=m[['AVUV','AVDV','IJR','VSS']].pct_change(fill_method=None)
sv=pd.DataFrame({'ret':.5*(svr.AVUV+svr.AVDV),'ctl':.5*(svr.IJR+svr.VSS)})
p36x=p36()
variants={'INCUMBENT_MOM_TREND':p64(('mom','trend')),'MOM_ONLY':p64(('mom',)),'TREND_ONLY':p64(('trend',))}

def evaluate(p64x):
    q=sv.rename(columns={'ret':'sv','ctl':'svctl'}).join(p64x.rename(columns={'ret':'p64','ctl':'p64ctl'})).join(p36x.rename(columns={'ret':'p36','ctl':'p36ctl'})).dropna().loc[:'2026-08-31']
    q['ret']=(q.sv+q.p64+q.p36)/3; q['ctl']=(q.svctl+q.p64ctl+q.p36ctl)/3; q['excess']=q.ret-q.ctl
...
challengers=['MOM_ONLY','TREND_ONLY']; best=max(challengers,key=lambda k:res[k]['annualized_mean_matched_excess'])
promote=bool(res[best]['annualized_mean_matched_excess']>inc['annualized_mean_matched_excess'] and res[best]['positive_blocks']>=inc['positive_blocks'] and res[best]['positive_blocks']>=2)
decision=f'P249_P64_INFORMATION_ABLATION__{best}__CHALLENGER' if promote else 'P249_P64_INFORMATION_ABLATION__INCUMBENT_INFORMATION_SET_SURVIVES'
