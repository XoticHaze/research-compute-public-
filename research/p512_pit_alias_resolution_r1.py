...
for _,r in pit[pit.ticker_norm.isin(pit_only)].iterrows():
 ...
 x=best[2]
 rows.append({'pit_ticker':js(r.ticker_norm),'pit_name':js(r.name_raw),'pit_cik_unsafe':js(r.pit_cik),'historical_ticker':js(x.ticker_norm) if accepted else None,'historical_name':js(x.name_raw) if accepted else None,'historical_cik':js(x.wiki_cik) if accepted else None,'name_similarity':round(float(best[1]),6),'runner_up_similarity':round(float(second[1]),6),'unique_margin':round(float(unique_margin),6),'match_class':'EXACT_NORMALIZED_NAME' if accepted and best[0] else ('HIGH_CONFIDENCE_UNIQUE_NAME' if accepted else 'UNRESOLVED')})
