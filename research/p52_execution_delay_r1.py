...
def run(delay):
    close=base.load(SYMS).sort_index(); last=pd.Timestamp(close.index.max()); last=last.tz_localize(None) if last.tzinfo else last; cutoff=last.to_period('M').start_time-pd.Timedelta(days=1)
    ...
    for i,dt in enumerate(m.index[:-1]):
        ...
        chosen=list(score.sort_values(ascending=False).head(3).index); w={s:(1/3 if s in chosen else 0.) for s in SYMS}; to=.5*sum(abs(w[s]-prev[s]) for s in SYMS)
...
def main():
    out={'schema':'research.p52_execution_delay_r1','parent':'P52','hypothesis':'The frozen momentum+trend top-3 independent-industry mechanism retains after-cost matched alpha when trade implementation is delayed 1/2/3/5 trading days after month-end signal observation.','scientific_contract':{'universe':list(SYMS),'factors':list(FACTORS),'top_k':3,'execution_delays_trading_days':list(DELAYS),'costs_bps':list(COSTS),'matched_control':'same-universe equal weight over identical delayed holding intervals','windows':WINDOWS,'chronological_folds':5,'complete_months_only':True,'no_parameter_tuning':True},'delays':{},'source':{'provider':'Yahoo Finance via yfinance; research-only'}}
    hashes={}; cutoffs={}
    for d in DELAYS:
    ...
    t=out['delays']['5']['2020_forward']['50']; out['decision']='P52_DELAY_ROBUSTNESS_SUPPORTED' if t['excess_cagr']>0 and t['positive_folds']>=3 else 'P52_IMPLEMENTATION_DELAY_WEAKNESS'
    Path('artifacts').mkdir(exist_ok=True); Path('artifacts/p52_execution_delay_r1.json').write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False)); print(json.dumps(out,sort_keys=True))
