...
OUTPUT = Path("artifacts/forward_senior_clo_current_core_shadow_r1.json")
WORKLOAD_ID = "FORWARD_SENIOR_CLO_CURRENT_CORE_SHADOW_R1"
MIX = 0.50
...
def _simulate(core: pd.Series, alt: pd.Series) -> tuple[pd.Series, list[dict[str, object]]]:
    ...
    for i, (dt, row) in enumerate(q.iterrows()):
        ...
        if last_month is not None and month != last_month:
            traded = abs(w_core - MIX) + abs(w_alt - (1 - MIX))
            rebalance_cost = MONTHLY_REBALANCE_COST * traded
            w_core, w_alt = MIX, 1 - MIX
        ...
        rows.append({
            ...
            "monthly_rebalance_traded_notional": traded,
...
def main() -> None:
    ...
    challenger, challenger_rows = _simulate(core_daily, simple[CANDIDATE])
    matched, matched_rows = _simulate(core_daily, simple[CASH])
    ...
    result = {
        "schema": "research.forward_senior_clo_current_core_shadow_r1",
        "workload_id": WORKLOAD_ID,
        "parent": "SENIOR_CLO_CARRY_ALPHA / P249+P266",
        ...
        "source_pins": {
            "current_core_fit_merge": "534f16791f5a66e72c047edf801c986077429f70",
            "current_core_fit_run_id": 34588700439,
            "current_core_fit_artifact_id": 10194693432,
            "incumbent_observer": "research/p279_p249_p266_forward_shadow_observer_r1.py",
        },
        "current_positions": {
            "incumbent": core_current,
            "challenger": _scaled_positions(core_current, CANDIDATE) if core_current else {},
            "matched_control": _scaled_positions(core_current, CASH) if core_current else {},
        },
        "forward": {
            ...
            "incumbent": _metrics(core_daily, include_exit_cost=False),
            "challenger_if_flattened_now": _metrics(challenger),
            "matched_control_if_flattened_now": _metrics(matched),
