...
def main():
    f,cc,ic,cutoff=p64.build(); blend=.5*f.cross+.5*f.industry; matched=.5*f.cross_ctrl+.5*f.industry_ctrl; qqq=f.qqq
    ...
    for name,start in WINDOWS.items():
        q=pd.DataFrame({'candidate':blend,'matched':matched,'qqq':qqq}).loc[pd.Timestamp(start):].dropna(); cm,mm,qm=metrics(q.candidate),metrics(q.matched),metrics(q.qqq)
    ...
    r=out['tests']['2022_forward']; out['decision']='P64_RECENT_RISK_ADJUSTED_OPPORTUNITY_SUPPORTED' if r['sharpe_delta_vs_qqq']>0 and r['calmar_delta_vs_qqq']>0 and r['cagr_excess_vs_matched']>0 else 'P64_RISK_ADJUSTED_OPPORTUNITY_NOT_SUPPORTED'
