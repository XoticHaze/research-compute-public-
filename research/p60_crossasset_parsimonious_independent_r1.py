...
SYMS=("SPY","QQQ","IEF","IAU","PDBC")

def run(lag=0):
    close=base.load(SYMS); m=close.resample("ME").last()
    mom=m.pct_change(6); trend=(close/close.rolling(200,min_periods=160).mean()-1).resample("ME").last()
    ...
    for dt in m.index:
        ...
        chosen=sc.sort_values(ascending=False).head(2).index.tolist(); w={s:(0.5 if s in chosen else 0.0) for s in SYMS}
        to=0.5*sum(abs(w[s]-prev[s]) for s in SYMS)
        rec.append({"date":nxt,"gross":sum(w[s]*float(r[s]) for s in SYMS),"ew":float(r.mean()),"turnover":to}); prev=w
...
def main():
    full,close=run(0); lag,_=run(1); tests={"full":pack(full),"one_month_execution_lag":pack(lag)}
    for start in ("2015-01-01","2020-01-01"): tests[f"{start[:4]}_forward"]=pack(full.loc[pd.Timestamp(start):])
    out={"schema":"research.p60_crossasset_parsimonious_independent_r1","parents":["P46","P57"],"hypothesis":"The fixed P57 momentum+trend top-2 cross-asset mechanism transports to an independent proxy representation without relying on long-duration Treasury, gold, or broad-commodity ETF identity.","scientific_contract":{"universe":list(SYMS),"factors":["mom6","trend200"],"top_k":2,"cadence":"monthly","costs_bps":[25,50],"matched_comparator":"same-universe equal weight","temporal_holdouts":["2015-forward","2020-forward"],"execution_lag_test":"one month","no_parameter_tuning":True},"source":{"provider":"Yahoo Finance via yfinance","normalized_price_panel_sha256":base.source_hash(close)},"tests":tests}
    Path("artifacts").mkdir(exist_ok=True); Path("artifacts/p60_crossasset_parsimonious_independent_r1.json").write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False))
