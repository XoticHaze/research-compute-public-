...
def st(s,switches=None):
 ...
 w=(1+x).cumprod();n=len(x);v=x.std(ddof=1)*math.sqrt(12);return {'cagr':float(w.iloc[-1]**(12/n)-1),'sharpe':float(x.mean()*12/v),'max_drawdown':float((w/w.cummax()-1).min())}
def ev(z):
 ...
 A=st(a,sw);I=st(i);AC=st(ac,sw);IC=st(ic);A['matched_excess']=A['cagr']-AC['cagr'];I['matched_excess']=I['cagr']-IC['cagr'];D={k:A[k]-I[k] for k in ['cagr','sharpe','max_drawdown']};return {'regime':A,'intl':I,'regime_control':AC,'intl_control':IC,'regime_vs_intl':D,'wins':sum(D[k]>0 for k in D)}
...
out={'schema':'research.p472_exus_regime_vs_intl_sleeve_r1.v1','workload_id':'P472_EXUS_REGIME_VS_INTL_SLEEVE_R1','parent':'DEVELOPED_EXUS_VALUE_FACTOR_ALPHA','claim':'Same fixed 25% incremental capital beyond frozen P249+P373 core: compare P470 FNDF/VEA weak-dollar regime implementation against confirmed 50/50 AVDV+IDMO return-enhancement sleeve. No weight/regime tuning.','contract':{'incremental_budget':.25,'regime_signal':'prior completed 12m UUP return < 0 shifted one month','switch_cost_bps':10,'endpoint_cost_bps':25,'chronology_blocks':3,'support':'regime has positive matched excess, beats AVDV+IDMO in >=2 of CAGR/Sharpe/maxDD full sample and >=2/3 blocks, and positive matched excess in >=2/3 blocks'},'coverage':{'months':n,'start':str(q.index[0].date()),'end':str(q.index[-1].date())},'full':full,'blocks':blocks,'regime_win_blocks':wb,'positive_matched_excess_blocks':pe,'decision':dec,'boundaries':{'scientific_authority':True,'portfolio_ranking':False,'allocation_authority':False,'runtime':False,'broker':False,'live_trading':False}}
