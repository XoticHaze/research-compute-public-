...
UA='CommandCenter MarketResearch P492 research@example.invalid';YEARS=list(range(2018,2025));N=40;COST=.001
OUT=Path('research/artifacts/p492_pit_balance_sheet_deleveraging_r1.json');OUT.parent.mkdir(parents=True,exist_ok=True)
def hist(target):
...
def feat(facts,asof):
 g=facts.get('facts',{}).get('us-gaap',{});a=instant(g,'Assets',asof);l=instant(g,'Liabilities',asof)
 common=sorted(set(x[0] for x in a)&set(x[0] for x in l))
 if len(common)<2:return None
 ratios=[]
 for end in common:
  av=[x for x in a if x[0]==end][-1][2];lv=[x for x in l if x[0]==end][-1][2]
  if av>0:ratios.append((end,lv/av))
 if len(ratios)<2:return None
 prev,cur=ratios[-2],ratios[-1];return prev[1]-cur[1]
def ret1(t,y):
 ...
 for _,row in s.iterrows():
  ...
  try:
   ...
   if f is not None:
    ...
    if rr is not None:vals.append({'ticker':row.ticker,'cik':str(cik),'deleveraging':f,'forward_return':rr})
  except Exception:pass
 coverage=len(vals)/len(s);ready=coverage>=.80
 if ready:
  v=sorted(vals,key=lambda x:x['deleveraging'],reverse=True);k=max(1,int(np.ceil(.30*len(v))));sel=v[:k];sr=float(np.mean([x['forward_return'] for x in sel])-COST);ctrl=float(np.mean([x['forward_return'] for x in v])-COST);spy=ret1('SPY',y);spy=(spy-COST) if spy is not None else None
...
out={'schema':'research.p492_pit_balance_sheet_deleveraging_r1.v1','workload_id':'P492_PIT_BALANCE_SHEET_DELEVERAGING_R1','parent':'PIT_FUNDAMENTAL_CROSS_SECTIONAL_SELECTION','claim':'Filed-at-safe reduction in total liabilities/assets over the two latest annual 10-K balance sheets identifies balance-sheet improvement that improves one-year returns versus contemporaneous sample and SPY.','contract':{'years':YEARS,'sample_n':N,'sample_rule':'40 evenly spaced historical S&P members after ticker sort','feature':'prior annual Liabilities/Assets minus latest annual Liabilities/Assets using 10-K facts filed by June 30','selection':'top 30% largest deleveraging','common_sample_coverage_min':.80,'entry_cost_bps':10,'acceptance':'all cohorts coverage-ready; mean excess >0 vs sample and SPY; sample excess positive >=5/7 years','no_rescue':True},'cohorts':coh,'decision':'PIT_DELEVERAGING_ALPHA_SUPPORTED' if sup else ('PIT_DELEVERAGING_DATA_NOT_READY' if len(ready)<len(YEARS) else 'PIT_DELEVERAGING_ALPHA_NOT_SUPPORTED'),'scientific_consequence':'Qualify for independent validation.' if sup else 'Reject exact mechanism if coverage complete; otherwise preserve as data-not-ready without threshold/date/feature rescue.','boundaries':{'portfolio_ranking':False,'allocation_authority':False,'runtime':False,'broker':False,'live_trading':False}}
