...
T=['XSMO','IJR','MTUM']; START=1514764800; END=1893456000; COST=0.001
...
def main():
 d={t:mr(fetch(t)) for t in T}; months=sorted(set(d['XSMO'])&set(d['IJR'])&set(d['MTUM'])); rows=[(m,d['XSMO'][m]-COST/12,d['IJR'][m],d['MTUM'][m]) for m in months]; years=sorted(set(m[0] for m in months)); held=[]; yr={}
 for y in years:
 ...
 if not held:
  ...
  full=100*12*statistics.mean(v for _,v in held); recent=[v for m,v in held if m>=(2022,1)]; recent_a=100*12*statistics.mean(recent) if recent else None; metrics={'months':len(held),'years':len(yr),'full_residual_alpha_pct':full,'recent_2022_residual_alpha_pct':recent_a,'positive_year_fraction':sum(v>0 for v in yr.values())/len(yr),'positive_years':sum(v>0 for v in yr.values()),'worst_year_residual_pct':min(yr.values())}; passes={k:metrics[k]>=v for k,v in G.items()}; out={'schema':'cc.market_research.xsmo_crossfit_alpha.r1','claim':'XSMO retains durable implementation alpha beyond small-cap beta and generic momentum exposure','inherited_learning':['P415 supported XSMO versus size-matched IJR with one negative chronology block preserved','academic small-cap momentum independently supports the mechanism while DWAS product transport failed'],'frozen':{'target':'XSMO','factors':['IJR','MTUM'],'leave_one_calendar_year_out':True,'cost_annual':COST,'gates':G},'metrics':metrics,'passes':passes,'decision':'PROMOTE' if all(passes.values()) else 'REJECT_NO_RESCUE','year_residual_alpha_pct':yr}
 open('xsmo-crossfit-alpha-r1-result.json','w').write(json.dumps(out,indent=2)); print(json.dumps(out,indent=2))
