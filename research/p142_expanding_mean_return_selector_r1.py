...
def build(assets):
    ...
    for i in range(36,len(m.index)-1):
        nxt=m.index[i+1]; hist=ret.iloc[1:i+1][list(assets)].dropna(how="any"); nr=ret.loc[nxt,list(syms)]
        if len(hist)<36 or nr.isna().any(): continue
...
def main():
    out={"schema":"research.p142_expanding_mean_return_selector_r1","contract":{"mechanism":"expanding prior-only unconditional historical monthly mean return top2","minimum_prior_months":36,"representations":{k:list(v) for k,v in p137.REPRESENTATIONS.items()},"windows":["full","2018-01-31","2022-01-31"],"costs_bps":[25,50],"matched_control":"same-universe equal weight","opportunity_cost":["SPY","QQQ"],"chronological_folds":3,"concentration_test":"remove five strongest selector-minus-equal-weight months","no_future_rows_or_parameter_topk_threshold_tuning":True},"results":{}}
    ...
    out["decision"]="EXPANDING_MEAN_RETURN_BROAD_DURABLE_ALPHA_SUPPORTED" if all(cells) and all(fo) and all(tr) else ("EXPANDING_MEAN_RETURN_ALPHA_MIXED_OR_CONCENTRATED" if sum(cells)>=8 and sum(fo)>=4 else "EXPANDING_MEAN_RETURN_NOT_BROADLY_SUPPORTED")
    Path("artifacts").mkdir(exist_ok=True); Path("artifacts/p142_expanding_mean_return_selector_r1.json").write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False)); c={r:{s:{"25_ew":out["results"][r][s]["25"]["excess_vs_equal_weight"],"50_ew":out["results"][r][s]["50"]["excess_vs_equal_weight"],"25_spy":out["results"][r][s]["25"]["excess_vs_spy"],"25_qqq":out["results"][r][s]["25"]["excess_vs_qqq"],"25_folds":out["results"][r][s]["25"]["positive_folds_vs_equal_weight"],"25_trim":out["results"][r][s]["25"]["five_best_relative_months_removed"],"turn":out["results"][r][s]["25"]["annual_turnover"]} for s in ("full","2018-01-31","2022-01-31")} for r in p137.REPRESENTATIONS}; print(json.dumps({"decision":out["decision"],"results":c},sort_keys=True))
