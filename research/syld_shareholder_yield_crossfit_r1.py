...
C=json.loads(Path('research/syld-shareholder-yield-crossfit-r1.json').read_text());START=dt.datetime.fromisoformat(C['start']+'T00:00:00+00:00');END=dt.datetime.now(dt.timezone.utc)+dt.timedelta(days=1)
def fetch(s):
...
def ols(rs):
 ...
 for r in rs:
  x=[1,r['SPY'],r['VTV'],r['QUAL']];y=r['SYLD']
  for i in range(p):
   xy[i]+=x[i]*y
...
for y in sorted({r['month'][:4] for r in rows}):
 ...
 b=ols(tr)
 for r in te:held.append({'month':r['month'],'residual':r['SYLD']-(b[0]+b[1]*r['SPY']+b[2]*r['VTV']+b[3]*r['QUAL'])-cost})
yr={}
for r in held:yr.setdefault(r['month'][:4],[]).append(r['residual'])
yc={y:comp(v) for y,v in yr.items()};res=[r['residual'] for r in held];rec=[r['residual'] for r in held if r['month']+'-01'>=C['recent_start']];net=[r['SYLD']-cost for r in rows];m={'months':len(rows),'full_syld_net_cagr':ann(net),'full_spy_cagr':ann([r['SPY'] for r in rows]),'full_excess_vs_spy':ann(net)-ann([r['SPY'] for r in rows]),'full_crossfit_residual_annualized':ann(res),'recent_crossfit_residual_annualized':ann(rec),'positive_heldout_year_fraction':sum(v>0 for v in yc.values())/len(yc),'worst_heldout_year_residual':min(yc.values()),'heldout_year_residuals':yc};g=C['gates'];ch={'full':m['full_crossfit_residual_annualized']>=g['min_full_residual'],'recent':m['recent_crossfit_residual_annualized']>=g['min_recent_residual'],'years':m['positive_heldout_year_fraction']>=g['min_positive_year_fraction'],'worst':m['worst_heldout_year_residual']>=g['min_worst_year'],'spy':m['full_excess_vs_spy']>=g['min_full_excess_vs_spy']};out={'schema':'syld_shareholder_yield_crossfit_result.v1','experiment_id':C['experiment_id'],'metrics':m,'checks':ch,'decision':'PASS_IMPLEMENTATION_DISCRIMINATOR' if all(ch.values()) else 'REJECT_IMPLEMENTATION_CLAIM','research_only':True};Path('syld-shareholder-yield-crossfit-r1-result.json').write_text(json.dumps(out,indent=2,sort_keys=True));print(json.dumps(out,sort_keys=True))
