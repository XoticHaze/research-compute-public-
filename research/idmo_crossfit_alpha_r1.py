...
T=['IDMO','EFA','IMTM']; START=1420070400; END=1893456000; COST=0.001
...
def main():
 d={t:mr(fetch(t)) for t in T}; months=sorted(set(d['IDMO'])&set(d['EFA'])&set(d['IMTM'])); rows=[(m,d['IDMO'][m]-COST/12,d['EFA'][m],d['IMTM'][m]) for m in months]; years=sorted(set(m[0] for m in months)); held=[]; yr={}
 for y in years:
 ...
 if not held: out={'decision':'UNSCORED_INSUFFICIENT_CROSSFIT_HISTORY','months':len(months)}
  ...
  full=100*12*statistics.mean(v for _,v in held); recent=[v for m,v in held if m>=(2022,1)]; metrics={'months':len(held),'years':len(yr),'full_residual_alpha_pct':full,'recent_2022_residual_alpha_pct':100*12*statistics.mean(recent),'positive_year_fraction':sum(v>0 for v in yr.values())/len(yr),'positive_years':sum(v>0 for v in yr.values()),'worst_year_residual_pct':min(yr.values())}; passes={k:metrics[k]>=v for k,v in G.items()}; out={'schema':'cc.market_research.idmo_crossfit_alpha.r1','claim':'IDMO retains durable implementation alpha beyond international beta and generic international momentum exposure','inherited_learning':['cross-representation concentration result where IDMO retained +1.09 pp trimmed matched excess and 3/3 chronology blocks','CC-RF-LEARN-XMMO-CROSSFIT-ALPHA-20260912-R1'], 'frozen':{'target':'IDMO','factors':['EFA','IMTM'],'leave_one_calendar_year_out':True,'cost_annual':COST,'gates':G},'metrics':metrics,'passes':passes,'decision':'PROMOTE' if all(passes.values()) else 'REJECT_NO_RESCUE','year_residual_alpha_pct':yr}
 open('idmo-crossfit-alpha-r1-result.json','w').write(json.dumps(out,indent=2)); print(json.dumps(out,indent=2))
