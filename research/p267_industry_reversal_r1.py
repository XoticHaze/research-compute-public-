...
def build(close):
    ret=close[INDUSTRIES].pct_change(); signal=ret[INDUSTRIES].shift(1)
    w=pd.DataFrame(0.0,index=close.index,columns=INDUSTRIES)
    for dt,row in signal.iterrows():
        if row.notna().sum()==len(INDUSTRIES): w.loc[dt,row.nsmallest(TOPK).index]=1/TOPK
...
res={'schema':'research.p267_industry_reversal_r1','parent':'P267','claim':'A prospectively fixed one-month cross-sectional reversal selector over diversified US industry ETFs creates durable after-cost excess versus a same-universe equal-weight control by holding the three prior-month laggards for the next month.','contract':{'universe':INDUSTRIES,'signal':'prior calendar-month total return, ascending','top_k':TOPK,'holding':'next month equal-weight prior-month laggards','cost_bps_per_one_way_turnover':COST_BP,'matched_control':'same-universe equal weight','opportunity_context':'SPY','windows':WINDOWS,'folds':5,'gate':'positive matched excess all windows; >=4/5 positive full-history folds; maxDD no worse than matched by >5pp','no_universe_horizon_topk_window_threshold_or_parameter_search':True},'tests':out,'decision':'P267_INDUSTRY_REVERSAL_SUPPORTED' if support else 'P267_INDUSTRY_REVERSAL_NOT_SUPPORTED','limitations':['ETF adjusted prices are research-only','high turnover is charged prospectively at 50 bps per one-way turnover but tax/market-impact effects are not modeled'],'boundaries':{'portfolio_ranking':False,'allocation_authority':False,'runtime':False,'broker':False,'live_trading':False}}
Path('artifacts').mkdir(exist_ok=True); Path('artifacts/p267_industry_reversal_r1.json').write_text(json.dumps(res,indent=2,sort_keys=True,allow_nan=False)); print(json.dumps(res,sort_keys=True))
