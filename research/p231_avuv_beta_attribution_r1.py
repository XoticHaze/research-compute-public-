...
def ev(d):
 regs={b:ols(c(d['AVUV']),c(d[b])) for b in ['IJR','IWM','SPY']}; fs=[]
 for i,ix in enumerate(np.array_split(np.arange(len(d)),5),1):
  q=d.iloc[ix]; z=ols(c(q['AVUV']),c(q['IJR'])); fs.append({'fold':i,'ijr_alpha_ann':z['alpha_ann'],'beta':z['beta']})
 return {'regressions':regs,'positive_ijr_alpha_folds':sum(x['ijr_alpha_ann']>0 for x in fs),'folds':fs}
r0=yf.download(A,start='2019-10-01',end='2026-09-03',auto_adjust=True,progress=False,group_by='column',threads=False); cl=r0['Close'][A] if isinstance(r0.columns,pd.MultiIndex) else r0[A]; cl=cl.dropna(how='any').resample('ME').last(); r=cl.pct_change().dropna(how='any'); t={k:ev(r.loc[pd.Timestamp(v):]) for k,v in W.items()}; ok=all(t[k]['regressions']['IJR']['alpha_ann']>0 for k in W) and t['2020']['positive_ijr_alpha_folds']>=4 and t['2020']['regressions']['IJR']['alpha_t_iid']>=1.5; out={'schema':'research.p231_avuv_beta_attribution_r1','parent':'P231','contract':{'candidate':'AVUV','primary_factor':'IJR','references':['IWM','SPY'],'windows':W,'folds':5,'cost_bps':B,'fixed_test':True,'regression':'monthly OLS candidate=alpha+beta*factor'},'tests':t,'decision':'P231_SUPPORT' if ok else 'P231_NOT_SUPPORTED','limitations':['IID alpha t-stat descriptive, not HAC','single-factor attribution is not a full factor model'],'boundaries':{'portfolio_ranking':False,'live_trading':False}}; Path('artifacts').mkdir(exist_ok=True); Path('artifacts/p231_avuv_beta_attribution_r1.json').write_text(json.dumps(out,indent=2,sort_keys=True)); print(json.dumps(out,sort_keys=True))