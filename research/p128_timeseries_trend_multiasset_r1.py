...
U=['SPY','IEF','GLD']; COSTS=(10,25,50); LOOK=10
...
def ev(q):
 ...
 return out
def main():
 raw=yf.download(U,start='2003-01-01',auto_adjust=True,progress=False,threads=False)['Close'].dropna().astype(float); last=pd.Timestamp(raw.index.max()); last=last.tz_localize(None) if last.tzinfo else last; cutoff=last.to_period('M').start_time-pd.Timedelta(days=1); m=raw.resample('ME').last(); m=m[m.index<=cutoff]; rows=[]; prev=np.zeros(len(U))
 for i in range(LOOK+1,len(m)-1):
  ma=m[U].iloc[i-LOOK:i].mean().to_numpy(); px=m[U].iloc[i-1].to_numpy(); active=(px>ma).astype(float); w=active/len(U); rr=np.array([float(m[s].iloc[i+1]/m[s].iloc[i]-1) for s in U]); turn=float(.5*np.abs(w-prev).sum()); rows.append({'date':m.index[i+1],'gross':float(w@rr),'turn':turn,'ew':float(rr.mean()),'spy':rr[0],'6040':float(.6*rr[0]+.4*rr[1])}); prev=w
 ...
 out={'schema':'research.p128_timeseries_trend_multiasset_r1','parent':'P128','hypothesis':'A fixed time-series trend overlay across SPY, IEF and GLD creates durable after-cost excess beyond always-invested equal-weight multi-asset exposure.','contract':{'universe':U,'signal':'each sleeve receives one-third weight next month only when prior completed month price exceeds mean of prior ten completed month-end prices; otherwise that third is cash at zero','allocation':'independent one-third sleeves','cost_bps_turnover':list(COSTS),'matched_control':'always-invested equal-weight SPY/IEF/GLD','secondary_control':'60/40 SPY/IEF','opportunity_control':'SPY','windows':['2010','2015','2020'],'folds':5,'parameter_search':False},'source':{'provider':'Yahoo Finance via yfinance; research-only','last_complete_month_end':str(cutoff.date()),'panel_sha256':hashlib.sha256(m.reset_index().to_csv(index=False).encode()).hexdigest()},'tests':tests,'decision':'P128_TIMESERIES_TREND_SUPPORTED_FOR_FURTHER_FALSIFICATION' if ok else 'P128_TIMESERIES_TREND_NOT_SUPPORTED'}; Path('artifacts').mkdir(exist_ok=True); Path('artifacts/p128_timeseries_trend_multiasset_r1.json').write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False)); print(json.dumps(out,sort_keys=True,allow_nan=False))
