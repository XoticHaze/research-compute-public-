...
for name,start in WINDOWS.items():
 ...
 def stats(mask):
  z=q.loc[mask]; f=full.loc[mask]; w=wo.loc[mask]
  return {'months':int(mask.sum()),'smallvalue_mean':float(z.sv.mean()),'with_smallvalue_mean':float(f.mean()),'without_smallvalue_mean':float(w.mean()),'mean_delta':float((f-w).mean()),'smallvalue_positive_fraction':float((z.sv>0).mean()),'smallvalue_beats_without_fraction':float((z.sv>w).mean())}
