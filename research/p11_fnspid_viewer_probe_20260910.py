...
def get(path, params):
    ...
    req=urllib.request.Request(url,headers={'User-Agent':'market-research-p11-fnspid-probe/1.0','Accept':'application/json'})
    try:
...
if not spl['ok']:
    ...
    if not items:
        ...
        if not sf or not df:
            ...
            all_ok=True; missing=[]; explicit=0; naive=0; date_only=0
            for s in SYMBOLS:
                where=f'"{sf}" = \'{s}\''
                asc=get('/filter',{'dataset':DATASET,'config':cfg,'split':split,'where':where,'orderby':f'"{df}" ASC','offset':0,'length':1})
                desc=get('/filter',{'dataset':DATASET,'config':cfg,'split':split,'where':where,'orderby':f'"{df}" DESC','offset':0,'length':1})
                ...
                if asc['ok'] and desc['ok']:
                    ...
                    if not ar: missing.append(s)
                    av=ar[0].get('row',{}).get(df) if ar else None; dv=dr[0].get('row',{}).get(df) if dr else None
                    row['earliest_raw']=av; row['latest_raw']=dv
                    for v in [av,dv]:
                        t=str(v or '')
