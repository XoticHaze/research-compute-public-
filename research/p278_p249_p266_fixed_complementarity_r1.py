...
if not P248_PATH.is_file():
 raise RuntimeError('P278 requires admitted P248 source owner at research/p248_smallvalue_survivor_complementarity_r1.py')
...
def industry_sleeve():
 ...
 raw=yf.download(INDUSTRIES,start='2010-01-01',end='2026-09-03',auto_adjust=True,progress=False,threads=False); close=raw['Close'] if isinstance(raw.columns,pd.MultiIndex) else raw
 m=close[INDUSTRIES].dropna().resample('ME').last(); ret=m.pct_change(); score=m.shift(1)/m.shift(12)-1; w=pd.DataFrame(0.0,index=m.index,columns=INDUSTRIES)
...
p64=p248.p64.copy(); p64.index=p64.index.to_period('M').to_timestamp('M'); p36=p248.p36.copy(); p36.index=p36.index.to_period('M').to_timestamp('M'); ind=industry_sleeve(); ind.index=ind.index.to_period('M').to_timestamp('M')
...
for name,start in WINDOWS.items():
 ...
 coren=ep(core,SV_BP/3); coremn=ep(corem,SV_BP/3); satn=ep(sat,SV_BP/4); satmn=ep(satm,SV_BP/4); cm=metrics(coren); sm=metrics(satn); cmm=metrics(coremn); smm=metrics(satmn); folds=[]
...
a=out['2020']; b=out['2022']; ai=a['satellite_increment']; bi=b['satellite_increment']; support=(ai['cagr_delta']>0 and bi['cagr_delta']>0 and ai['positive_calendar_folds']>=3 and bi['positive_calendar_folds']>=3 and ai['maxdd_delta']>=0 and bi['maxdd_delta']>=0 and ai['worst_rolling_12m_delta']>=0 and bi['worst_rolling_12m_delta']>=0)
