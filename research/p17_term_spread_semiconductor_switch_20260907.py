...
import numpy as np
import pandas as pd
START='1999-01-01'; END='2026-09-08'; SYMBOLS=['SMH','QQQ','SPY']; LAG_DAYS=7; DELTA_OBS=63; COSTS=[10.0,25.0,50.0]; PRIMARY=25.0; OUT='p17-term-spread-semiconductor-switch-receipt.json'
TERM_REPO='yohan4477/semi-report'; TERM_COMMIT='ecd58c6b5f0d4c1f795ce484014db8f9120da449'; TERM_BLOB='c8c82485685f540ac74c778eab11dab946704bc4'; TERM_URL=f'https://raw.githubusercontent.com/{TERM_REPO}/{TERM_COMMIT}/data/fred/T10Y2Y.csv'
...
def term_spread():
 raw=fetch(TERM_URL); f=pd.read_csv(io.BytesIO(raw),na_values=['.'],comment='#'); dc='observation_date' if 'observation_date' in f else ('DATE' if 'DATE' in f else f.columns[0]); vc='T10Y2Y'; idx=pd.to_datetime(f[dc],utc=True,errors='coerce'); val=pd.to_numeric(f[vc],errors='coerce'); s=pd.Series(val.to_numpy(),index=idx,name=vc).dropna().sort_index(); s=s[~s.index.duplicated(keep='last')]
 if len(s)<12000 or s.index.max()!=pd.Timestamp('2026-09-04',tz='UTC'): raise RuntimeError(f'pinned T10Y2Y identity/coverage mismatch valid_rows={len(s)} last={s.index.max()}')
...
def main():
 ps={s:price(s) for s in SYMBOLS}; common=pd.DatetimeIndex(sorted(set.intersection(*[set(x.index) for x in ps.values()]))); frame=pd.DataFrame({s:x.reindex(common) for s,x in ps.items()}).dropna(); ts,term_sha=term_spread(); me=month_ends(frame); decisions=[]
