...
def build():
    close=base.load(ALL).sort_index(); m=close.resample('ME').last(); trend=(close[list(SECTORS)]/close[list(SECTORS)].rolling(TREND,min_periods=160).mean()).resample('ME').last(); mom=m[list(SECTORS)].pct_change(6)
    rec=[]; pg={}; pr={}
