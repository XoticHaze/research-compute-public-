...
LOOKBACK=20; H=20; DELAY=1; STEP=20; COSTS=(25.,50.,100.)
def maxdd(rs):
...
def evaluate(name,bench,stocks):
    ...
    for i in range(LOOKBACK+252,len(cal)-(H+DELAY),STEP):
        rel={s:float((px[s].iloc[i]/px[s].iloc[i-LOOKBACK]-1)-(px[bench].iloc[i]/px[bench].iloc[i-LOOKBACK]-1)) for s in stocks}; chosen=sorted(rel,key=rel.get)[:3]; e=i+DELAY; x=e+H
...
results={k:evaluate(k,*v) for k,v in FAMILIES.items()}; survivors=[k for k,v in results.items() if v.get('decision')=='SURVIVES_MATCHED_ALPHA_GATE']; out={'schema':'sector_relative_reversal_tournament.v1','contract':{'signal':'prior20d stock-minus-sector relative return; select bottom3','selection':'bottom3_of8','entry_delay_days':DELAY,'holding_days':H,'rebalance_step_days':STEP,'costs_bps':COSTS,'no_parameter_tuning':True},'results':results,'survivors':survivors,'decision':'HAS_SURVIVOR_REQUIRES_INDEPENDENT_HOLDOUT' if survivors else 'SECTOR_RELATIVE_REVERSAL_NOT_SUPPORTED','research_only':True,'promotion_authority':False,'allocation_authority':False,'live_trading_change':False}; open('/tmp/result.json','w').write(json.dumps(out,sort_keys=True,indent=2)+'\n'); print('SECTOR_RELATIVE_REVERSAL='+json.dumps(out,sort_keys=True))
