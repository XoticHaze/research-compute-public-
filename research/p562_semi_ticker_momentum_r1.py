...
NAMES=['ADI','AMD','AMAT','AVGO','KLAC','LRCX','MCHP','MRVL','MU','NVDA','QCOM','TXN']
...
def main():
    tickers=NAMES+['SMH'];raw=yf.download(tickers,start='2009-01-01',end='2026-09-12',auto_adjust=True,progress=False,group_by='column');c=raw['Close'] if isinstance(raw.columns,pd.MultiIndex) else raw;c=c[tickers].dropna(how='all').ffill();r=c.pct_change().dropna(how='all');m=c[NAMES].resample('ME').last();score=m.shift(1)/m.shift(12)-1;picks=score.rank(axis=1,ascending=False,method='first')<=3;w=picks.astype(float).div(picks.sum(axis=1),axis=0).fillna(0);dw=w.reindex(r.index,method='ffill').shift(1).fillna(0);name_r=r[NAMES].fillna(0);prev=dw.shift(1).fillna(0);turn=(dw-prev).abs().sum(axis=1)/2;strat=(dw*name_r).sum(axis=1)-turn*(COST_BPS/10000);eq=name_r.mean(axis=1);d=pd.DataFrame({'strategy':strat,'equal_universe':eq,'SMH':r.SMH,'turnover':turn,'names':(dw>0).sum(axis=1)});d=d.loc[dw.sum(axis=1)>0].dropna();overall=ev(d,'2011-01-01',END);folds=[ev(d,*f) for f in FOLDS];ep=sum(x['matched_excess_cagr']>0 for x in folds);sp=sum(x['smh_excess_cagr']>0 for x in folds);decision='P562_SUPPORTED' if overall['matched_excess_cagr']>0 and overall['smh_excess_cagr']>0 and ep>=2 and sp>=2 else 'P562_NOT_SUPPORTED_NO_RESCUE';out={'schema':'research.p562_semi_ticker_momentum_r1','parent':'P562','claim':'A frozen monthly 12-1 momentum selector across a fixed liquid semiconductor stock universe can create durable after-cost excess over both the equal-weight same-name universe and SMH.','frozen_contract':{'universe':NAMES,'universe_note':'fixed present-day liquid semiconductor names; survivorship risk acknowledged, so alpha gate is relative to same fixed-universe control as well as SMH','selection':'top 3 by trailing 12-month return excluding latest month','rebalance':'monthly, applied following session','cost_bps_per_one_way_turnover':COST_BPS,'controls':['equal-weight same fixed universe','SMH'],'folds':FOLDS,'gate':'positive aggregate excess vs both controls and >=2/3 positive folds vs each','no_parameter_rescue':True},'overall':overall,'folds':folds,'positive_fold_count_vs_equal_universe':ep,'positive_fold_count_vs_smh':sp,'decision':decision,'boundaries':{'portfolio_ranking':False,'allocation_authority':False,'runtime':False,'broker':False,'live_trading':False}};Path('artifacts').mkdir(exist_ok=True);Path('artifacts/p562_semi_ticker_momentum_r1.json').write_text(json.dumps(out,indent=2,sort_keys=True));print(json.dumps(out,sort_keys=True))
