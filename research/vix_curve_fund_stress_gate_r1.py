...
def main():
    ...
    model=[]; basket=[]; spy=[]; prev=None; stress_days=0
    for d in days:
        stress=p['^VIX'][d]>=p['^VIX3M'][d]
        if stress: stress_days+=1
        switch=prev is not None and stress!=prev; prev=stress
        m=sum((0 if stress else r[s][d]) for s in FUNDS)/len(FUNDS) - (COST if switch else 0)
    ...
    out={'schema':'research.vix_curve_fund_stress_gate.v1','semantic_id':SEMANTIC_ID,'claim_tested':'A structurally inverted VIX/VIX3M curve identifies stress periods strongly enough that moving a frozen QQQ/IWM/HYG basket to cash during inversion creates durable after-cost excess return.','inheritance':{'unresolved_uncertainty':'whether options-implied market stress contains fund-level timing information after corporate-event and price-only stock-selection families failed'},'frozen_spec':{'funds':FUNDS,'stress_rule':'VIX >= VIX3M','portfolio':'equal-weight QQQ/IWM/HYG when curve is not inverted; cash when inverted','cost_per_state_switch_bps':5,'forbidden_parameter_rescue':['curve threshold','fund panel','start date','cost','recent cutoff','post-hoc year exclusions']},'metrics':metrics,'promotion_gates':gates,'decision':decision,'strongest_competing_explanation':'VIX-curve inversion is contemporaneous risk information whose avoided losses are offset by missed rebounds and switching opportunity cost.','forward_eligibility':decision.startswith('PROMOTE'),'generated_at_utc':datetime.now(timezone.utc).isoformat()}
