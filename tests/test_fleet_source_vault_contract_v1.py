...
class FleetSourceVaultContractTests(unittest.TestCase):
    ...
    def test_private_source_contract_test_is_exact_sha_scoped_and_expires(self):
        text = SOURCE.read_text(encoding="utf-8")
        self.assertIn("PRIVATE_SOURCE_CONTRACT_TEST_IDENTITY", text)
        self.assertIn(
            "mmibkr-private-source-contract-test-r1.yml@refs/heads/main",
            text,
        )
        self.assertIn(
        ...
        self.assertIn("PRIVATE_SOURCE_CONTRACT_TEST_EXPIRES_AT", text)
        self.assertIn("2026-09-23T12:00:00Z", text)
        self.assertIn("matchedPrivateSourceContractTest", text)
        self.assertIn(
